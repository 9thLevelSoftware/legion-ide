//! GP-1 golden-path smoke orchestration.
//!
//! `xtask golden-path-1` spawns the `golden_path_1` binary from `legion-app`
...
//! `target/golden-path/gp1_report.toml`; this command prints a summary and
//! forwards the subprocess exit code.

use std::{path::Path, process};

/// Options for the `golden-path-1` subcommand.
#[derive(Debug)]
pub struct GoldenPath1Options {
    ...
    pub out_dir: String,
    /// If `Some`, also copy the evidence TOML to this path
    /// (`plans/evidence/production/M8/` by convention) after a successful run.
    pub record_evidence: Option<String>,
}

impl Default for GoldenPath1Options {
    fn default() -> Self {
        Self {
...
/// Run `golden-path-1`: compile and spawn the legion-app binary subprocess.
///
...
pub fn run_golden_path_1(workspace_root: &Path, opts: &GoldenPath1Options) -> i32 {
    // Resolve paths relative to the workspace root.
    let fixture_dir = workspace_root.join(&opts.fixture_dir);
    if !fixture_dir.is_dir() {
        eprintln!(
            "golden-path-1: fixture directory not found: {}",
            fixture_dir.display()
        );
        return 2;
    }
    ...
    eprintln!(
        "golden-path-1: spawning subprocess: cargo {}",
        cargo_args.join(" ")
    );
    ...
    match status {
        ...
        Err(err) => {
            eprintln!("golden-path-1: failed to spawn cargo: {err}");
            1
        }
    }
}
