...
- Define a route matrix for [`ProposalExecutionRoute`](../crates/legion-app/src/lib.rs:123).
...
1. Manual save is already proposal mediated. [`SaveWorkflowService::save_active_buffer()`](../crates/legion-app/src/lib.rs:1321) requests editor save text, builds a save proposal, validates and previews it, then writes through [`WorkspaceActor::save_file_with_proposal()`](../crates/legion-project/src/lib.rs:1622). Rejected outcomes are returned through [`AppSaveOutcome::Rejected`](../crates/legion-app/src/lib.rs:1990), preserving dirty editor text.
2. The app has a generalized routing classifier but not generalized execution. [`ProposalExecutionRoute`](../crates/legion-app/src/lib.rs:123) classifies save, editor-buffer, workspace-file, terminal, batch, metadata-only, mixed, and unsupported routes through [`ProposalExecutionRoute::for_payload()`](../crates/legion-app/src/lib.rs:135).
3. Non-save execution is intentionally fail-closed today. [`AppProposalCoordinator::unsupported_response()`](../crates/legion-app/src/lib.rs:550) returns structured unsupported rejection metadata, and [`ProposalRequest::Apply`](../crates/legion-protocol/src/lib.rs:3739) is routed to that rejection path by [`ProposalPort`](../crates/legion-app/src/lib.rs:676) implementation logic at [`AppProposalCoordinator::handle()`](../crates/legion-app/src/lib.rs:677).
4. Affected-target traversal is present and total for existing payload DTOs. [`AppProposalCoordinator::affected_target_coverage()`](../crates/legion-app/src/lib.rs:350) and [`AppProposalCoordinator::visit_payload_targets()`](../crates/legion-app/src/lib.rs:373) cover text edit, file operation, save, format, code action, terminal, workspace edit, and batch payloads without relying on save-only file identity assumptions.
5. Protocol DTOs for the generalized substrate exist. [`WorkspaceProposal`](../crates/legion-protocol/src/lib.rs:1472), [`ProposalPayload`](../crates/legion-protocol/src/lib.rs:1507), [`ProposalTargetCoverage`](../crates/legion-protocol/src/lib.rs:1617), [`BatchProposalPayload`](../crates/legion-protocol/src/lib.rs:1630), [`ProposalRollbackStep`](../crates/legion-protocol/src/lib.rs:1715), [`ProposalPartialFailureRecord`](../crates/legion-protocol/src/lib.rs:1751), [`ProposalLifecycleState`](../crates/legion-protocol/src/lib.rs:1969), [`ProposalLifecycleCommand`](../crates/legion-protocol/src/lib.rs:2141), [`ProposalRequest`](../crates/legion-protocol/src/lib.rs:3728), and [`ProposalResponse`](../crates/legion-protocol/src/lib.rs:3747) define the required contract shape.
...
10. Dependency policy does not authorize an ad hoc LSP runtime. [`dependency-policy.md`](dependency-policy.md:88) limits Phase 3 internal activation for [`legion-index`](../crates/legion-index/Cargo.toml:1), and [`dependency-policy.md`](dependency-policy.md:113) states planned runtime surfaces are policy placeholders only until activation gates are satisfied.

## Route matrix for [`ProposalExecutionRoute`](../crates/legion-app/src/lib.rs:123)

| Route | Current status | Allowed now | Required future gate | Fail-closed outcome until gate |
| --- | --- | --- | --- | --- |
| [`ProposalExecutionRoute::SaveFile`](../crates/legion-app/src/lib.rs:125) | Implemented only through the manual save workflow using [`SaveWorkflowService::save_active_buffer()`](../crates/legion-app/src/lib.rs:1321). Generic [`ProposalRequest::Apply`](../crates/legion-protocol/src/lib.rs:3739) still rejects through [`AppProposalCoordinator::unsupported_response()`](../crates/legion-app/src/lib.rs:550). | Manual save validation, preview, audit emission, and workspace write through [`WorkspaceActor::save_file_with_proposal()`](../crates/legion-project/src/lib.rs:1622). | Generalized service must wrap the current save semantics without weakening expected fingerprint, file content version, workspace generation, buffer version, snapshot id, capability, principal, correlation, causality, conflict, and dirty-buffer guarantees. | Stale, denied, conflict, failed, or unsupported outcomes return rejected proposal metadata and preserve dirty editor text through [`AppSaveOutcome::Rejected`](../crates/legion-app/src/lib.rs:1990). |
| [`ProposalExecutionRoute::EditorBuffer`](../crates/legion-app/src/lib.rs:126) | Classified for [`ProposalTargetKind::OpenBuffer`](../crates/legion-protocol/src/lib.rs:1559) targets, but non-save validation and apply are rejected. | No generalized open-buffer mutation is allowed. | Must apply only through editor-owned transactions such as [`EditorEngine::apply_edit()`](../crates/legion-app/src/lib.rs:2092), with buffer version, snapshot id, undo or rollback group metadata, preview, approval, audit, and stale-response checks. | Reject unsupported, stale, missing-precondition, or policy-denied requests before text mutation. |
| [`ProposalExecutionRoute::WorkspaceFile`](../crates/legion-app/src/lib.rs:127) | Classified for [`ProposalTargetKind::ClosedFile`](../crates/legion-protocol/src/lib.rs:1561) and [`ProposalTargetKind::PathOnly`](../crates/legion-protocol/src/lib.rs:1563), but non-save execution is rejected. | No generalized closed-file create, delete, rename, format, code action, or workspace edit apply is allowed. | Must route through workspace VFS authority with capability checks, path policy, expected fingerprint or content preconditions, conflict detection, rollback records, audit-before-success, and dirty-open-buffer protection. | Reject before disk mutation if any precondition, capability, target coverage, rollback plan, conflict, or trust gate is missing. |
| [`ProposalExecutionRoute::Terminal`](../crates/legion-app/src/lib.rs:128) | Classified for [`ProposalTargetKind::TerminalSession`](../crates/legion-protocol/src/lib.rs:1565), but terminal apply is rejected. | No terminal command execution is allowed through proposals. | Requires a terminal runtime ADR, dependency-policy entry, capability contract, command preview model, bounded output policy, cancellation model, audit contract, and tests. | Reject as unsupported or policy-denied with metadata-only diagnostics; never spawn or write terminal state. |
| [`ProposalExecutionRoute::Batch`](../crates/legion-app/src/lib.rs:129) | [`ProposalPayload::Batch`](../crates/legion-protocol/src/lib.rs:1529) is classified and targets are derived, but runtime planning and apply are rejected. | Target derivation and deterministic preview metadata only. | Must validate [`ProposalBatchAtomicity`](../crates/legion-protocol/src/lib.rs:1532), [`ProposalBatchRollbackPolicy`](../crates/legion-protocol/src/lib.rs:1543), dependency edges, target coverage, rollback steps, and partial-failure records before any mutation. | Reject if declared atomicity cannot be honored, rollback metadata is incomplete, target coverage is partial without policy approval, or any item route lacks an accepted executor. |
| [`ProposalExecutionRoute::MetadataOnly`](../crates/legion-app/src/lib.rs:130) | Classified for [`ProposalTargetKind::MetadataOnly`](../crates/legion-protocol/src/lib.rs:1573), but non-save validation and apply are still rejected by current coordinator behavior. | Metadata-only target derivation can be previewed as data, but no generalized lifecycle apply is accepted. | May apply only after proving there is no editor, disk, terminal, plugin, remote, collaboration, or command side effect and after audit persistence succeeds. | Reject if the payload carries hidden mutation, command execution, incomplete redaction, missing correlation, missing causality, or storage failure. |
| [`ProposalExecutionRoute::Mixed`](../crates/legion-app/src/lib.rs:131) | Mixed editor, workspace, terminal, metadata, or other route coverage is classified but not executable. | No mixed-route apply is allowed. | Requires all contained routes to be individually accepted plus cross-authority ordering, preflight, rollback, partial-failure, conflict, and audit semantics. | Reject before mutation unless the generalized service can prove every route can prepare and either commit atomically or record exact declared partial-failure and rollback outcomes. |
| [`ProposalExecutionRoute::Unsupported`](../crates/legion-app/src/lib.rs:132) | Empty coverage and target kinds such as [`ProposalTargetKind::RemoteWorkspace`](../crates/legion-protocol/src/lib.rs:1567), [`ProposalTargetKind::CollaborationSession`](../crates/legion-protocol/src/lib.rs:1569), and [`ProposalTargetKind::Plugin`](../crates/legion-protocol/src/lib.rs:1571) are unsupported by classifier policy. | No apply, no side effects, and no runtime activation. | Requires accepted ADR, dependency-policy entry, protocol DTOs, capability contracts, ownership tests, redaction policy, and phase evidence for each runtime surface. | Reject as unsupported with metadata-only diagnostics. |
...
- UI remains projection-only. [`Shell`](../crates/legion-ui/src/ui.rs:228) may render proposal projection state and dispatch user intents, but must not apply proposals, mutate buffers, own workspace VFS state, or write files.
...
- Every mutation must enter as [`WorkspaceProposal`](../crates/legion-protocol/src/lib.rs:1472) and progress through explicit [`ProposalRequest`](../crates/legion-protocol/src/lib.rs:3728) transitions to [`ProposalResponse`](../crates/legion-protocol/src/lib.rs:3747).
- The service must enforce [`ProposalLifecycleState`](../crates/legion-protocol/src/lib.rs:1969) states as a real state machine, not independent helper calls.
- Apply must require validated and previewed state, approval when policy demands approval, non-expired proposal state through [`WorkspaceProposal::is_expired()`](../crates/legion-protocol/src/lib.rs:1501), and non-stale version context through [`WorkspaceProposal::is_stale()`](../crates/legion-protocol/src/lib.rs:1495).
...
- Batch validation must reject impossible atomicity claims before mutation using [`ProposalBatchAtomicity`](../crates/legion-protocol/src/lib.rs:1532), [`ProposalBatchRollbackPolicy`](../crates/legion-protocol/src/lib.rs:1543), [`ProposalRollbackStep`](../crates/legion-protocol/src/lib.rs:1715), and [`ProposalPartialFailureRecord`](../crates/legion-protocol/src/lib.rs:1751).
...
- Approval is an app/protocol lifecycle command through [`ProposalLifecycleCommand`](../crates/legion-protocol/src/lib.rs:2141); UI submits intent only.
...
- Closed-file mutation must use workspace VFS authority and preserve existing fail-closed write behavior from [`WorkspaceActor::save_file_with_proposal()`](../crates/legion-project/src/lib.rs:1622).
...
- Existing save event behavior through [`SaveWorkflowService::observe_proposal_response()`](../crates/legion-app/src/lib.rs:1431), [`event_metadata_record()`](../crates/legion-observability/src/lib.rs:376), and [`proposal_audit_record()`](../crates/legion-observability/src/lib.rs:394) is the baseline.
- Storage failure in an audit-required path must fail closed using [`ProposalFailureReason::StorageFailed`](../crates/legion-protocol/src/lib.rs:2032) or an equivalent typed failure.
...
- Unsupported routes return [`ProposalResponse::Rejected`](../crates/legion-protocol/src/lib.rs:3763) with [`ProposalRejectionReason::Unsupported`](../crates/legion-protocol/src/lib.rs:2006) or policy-denied metadata before any side effect.
...
- Formatting, rename, organize imports, quick fixes, refactors, and workspace edits from LSP must be translated into [`WorkspaceProposal`](../crates/legion-protocol/src/lib.rs:1472) payloads before preview, approval, or application.
...
- [x] Replace save-specific proposal orchestration with an app-domain generalized service while preserving [`SaveWorkflowService::save_active_buffer()`](../crates/legion-app/src/lib.rs:1321) caller behavior and [`WorkspaceActor::save_file_with_proposal()`](../crates/legion-project/src/lib.rs:1622) preconditions.
- [x] Make generic apply for [`ProposalPayload::SaveFile`](../crates/legion-protocol/src/lib.rs:1519) equivalent to the existing manual save path or explicitly keep generic apply denied with documented rationale until migration completes.
- [x] Implement deny-by-default validation for [`ProposalPayload::TextEdit`](../crates/legion-protocol/src/lib.rs:1511), [`ProposalPayload::CreateFile`](../crates/legion-protocol/src/lib.rs:1513), [`ProposalPayload::DeleteFile`](../crates/legion-protocol/src/lib.rs:1515), [`ProposalPayload::RenameFile`](../crates/legion-protocol/src/lib.rs:1517), [`ProposalPayload::FormatFile`](../crates/legion-protocol/src/lib.rs:1521), [`ProposalPayload::CodeAction`](../crates/legion-protocol/src/lib.rs:1523), [`ProposalPayload::WorkspaceEdit`](../crates/legion-protocol/src/lib.rs:1525), [`ProposalPayload::TerminalCommand`](../crates/legion-protocol/src/lib.rs:1527), and [`ProposalPayload::Batch`](../crates/legion-protocol/src/lib.rs:1529).
...
- Plugin, remote, and collaboration denial: target kinds represented by [`ProposalTargetKind::Plugin`](../crates/legion-protocol/src/lib.rs:1571), [`ProposalTargetKind::RemoteWorkspace`](../crates/legion-protocol/src/lib.rs:1567), and [`ProposalTargetKind::CollaborationSession`](../crates/legion-protocol/src/lib.rs:1569) reject before side effects.
...
| Protocol DTO contracts | `cargo test -p legion-protocol --test dto_contracts` | [`legion-protocol-dto-contracts.txt`](evidence/phase-3/legion-protocol-dto-contracts.txt) |
| Proposal routing regression | `cargo test -p legion-app --test workspace_vfs_integration workspace_vfs_integration_non_save_proposals_are_structurally_rejected_without_panic` | [`proposal-routing-regression.txt`](evidence/phase-3/proposal-routing-regression.txt) |
| Batch coverage regression | `cargo test -p legion-app --test workspace_vfs_integration workspace_vfs_integration_batch_affected_targets_are_visited_in_item_order` | [`proposal-routing-regression.txt`](evidence/phase-3/proposal-routing-regression.txt) |
| Save conflict regression | `cargo test -p legion-app --test workspace_vfs_integration workspace_vfs_integration_external_overwrite_between_open_and_save_yields_conflict` | [`save-conflict-regression.txt`](evidence/phase-3/save-conflict-regression.txt) |
| Index and semantic tests | `cargo test -p legion-index --all-targets` | [`legion-index-tests.txt`](evidence/phase-3/legion-index-tests.txt), [`lexical-symbol-map-tests.txt`](evidence/phase-3/lexical-symbol-map-tests.txt), [`tree-sitter-cache-tests.txt`](evidence/phase-3/tree-sitter-cache-tests.txt), [`normalized-graph-contract-tests.txt`](evidence/phase-3/normalized-graph-contract-tests.txt), and [`semantic-query-api-tests.txt`](evidence/phase-3/semantic-query-api-tests.txt) |
...
| Editor latency and background work | `cargo test -p legion-editor --test performance_suite -- --list` plus accepted Phase 3 performance runs | [`editor-semantic-latency.txt`](evidence/phase-3/editor-semantic-latency.txt) |
