...
The repository has successfully moved beyond the earliest spike hazards: the UI is now projection-only through [`Shell`](crates/legion-ui/src/ui.rs:228), editor and project do not have a direct crate dependency under [`dependency-policy.md`](plans/dependency-policy.md:22), application-level integration tests exercise workspace/editor routing in [`workspace_vfs_integration.rs`](crates/legion-app/tests/workspace_vfs_integration.rs:32), and the accepted phase zero gates passed in [`architecture-freeze-v0.1.md`](plans/architecture-freeze-v0.1.md:13).

> Historical rebaseline note (2026-05-15): the direct save-bypass concerns captured in this review describe the pre-rebaseline implementation. Current manual saves route through [`SaveWorkflowService::save_active_buffer()`](crates/legion-app/src/lib.rs:938) and then through [`WorkspaceActor::save_file_with_proposal()`](crates/legion-app/src/lib.rs:1021), while the shell remains projection-only through [`Shell`](crates/legion-ui/src/ui.rs:228). Treat the save-bypass language below as historical context unless it is explicitly discussing proposal generalization beyond save.
>
...
2. Protocol service ports exist, but core concrete services are mostly invoked directly rather than through implemented ports such as [`EditorPort`](crates/legion-protocol/src/lib.rs:1841), [`ProposalPort`](crates/legion-protocol/src/lib.rs:1847), and [`StorageRepositoryPort`](crates/legion-protocol/src/lib.rs:1877).
3. Observability has usable sink and envelope helpers through [`InMemoryEventSink`](crates/legion-observability/src/lib.rs:51) and [`EventEnvelopeBuilder`](crates/legion-observability/src/lib.rs:177), but the core app, editor, and workspace flows do not emit those events.
4. The text model still rebuilds a full text cache and line index after edits through [`TextBuffer::try_replace_range()`](crates/legion-text/src/lib.rs:820), so the large-file architecture remains intentionally incomplete.
5. Storage, session restore, and audit persistence are not yet wired into app/workspace flows despite storage contracts and local persistence scaffolding in [`SessionRecord`](crates/legion-storage/src/lib.rs:43).
...
The diagram above is historical for the save path. The current phase-zero baseline is safer because manual saves now flow through [`SaveWorkflowService::save_active_buffer()`](crates/legion-app/src/lib.rs:938), proposal validation/preview, and [`WorkspaceActor::save_file_with_proposal()`](crates/legion-app/src/lib.rs:1021). The remaining architecture gap is that proposal mediation is still save-focused rather than universal across future LSP, plugin, terminal, AI, collaboration, or remote mutation sources.
...
> Historical note (rebaselined 2026-05-15): this finding captured the pre-Track-5 save path. Current application saves no longer jump directly from app code to a raw workspace write. They route through [`SaveWorkflowService::save_active_buffer()`](crates/legion-app/src/lib.rs:938), emit proposal lifecycle events via [`SaveWorkflowService::observe_proposal_response()`](crates/legion-app/src/lib.rs:1048), and apply disk mutations through [`WorkspaceActor::save_file_with_proposal()`](crates/legion-app/src/lib.rs:1021). The remaining follow-on work is to generalize the same proposal discipline beyond manual save.
...
| Evidence | [`AppComposition::save_active_buffer()`](crates/legion-app/src/lib.rs:214) calls [`EditorEngine::request_save()`](crates/legion-editor/src/lib.rs:930) and then directly calls [`WorkspaceActor::write_file_text()`](crates/legion-project/src/lib.rs:1083). The target architecture requires a central pipeline in [`ide-core-architecture-spec-v0.1.md`](plans/ide-core-architecture-spec-v0.1.md:329), and phase six explicitly requires versioned, previewable, auditable durable mutation in [`foundational-core-ide-platform-implementation-plan-v0.1.md`](plans/foundational-core-ide-platform-implementation-plan-v0.1.md:152). |
...
| Recommendation | Introduce a proposal service implementing [`ProposalPort`](crates/legion-protocol/src/lib.rs:1847). Route saves by creating [`WorkspaceProposal`](crates/legion-protocol/src/lib.rs:783) with an expanded [`SaveFileProposal`](crates/legion-protocol/src/lib.rs:873), mandatory [`ProposalVersionPreconditions`](crates/legion-protocol/src/lib.rs:711), and conflict-safe expected fingerprint metadata. Make [`WorkspaceActor::write_file_text()`](crates/legion-project/src/lib.rs:1083) internal or require proposal context and precondition verification before it can write. |
...
| Evidence | [`FileConflictState`](crates/legion-protocol/src/lib.rs:502) is a generic struct with a free-form reason. [`ProposalResponse`](crates/legion-protocol/src/lib.rs:1568) only models valid, preview, applied, and denied. [`SaveFileProposal`](crates/legion-protocol/src/lib.rs:873) contains only file identity and snapshot id. Phase six requires clean, dirty, saving, save failed, disk changed clean, conflict dirty, reload available, keep-both pending, compare pending, plus proposal diagnostics and audit lifecycle in [`foundational-core-ide-platform-implementation-plan-v0.1.md`](plans/foundational-core-ide-platform-implementation-plan-v0.1.md:156). |
...
| Recommendation | Replace generic conflict reason modeling with a typed conflict state machine. Extend [`ProposalResponse`](crates/legion-protocol/src/lib.rs:1568) to include created, validated, approved, rejected, applied, failed, rolled back, stale, and conflict states. Extend [`SaveFileProposal`](crates/legion-protocol/src/lib.rs:873) with buffer version, file content version, workspace generation, expected fingerprint, save intent, trust decision, required capability, principal, correlation id, diagnostics, and conflict policy. |
...
| Evidence | Protocol declares [`WorkspacePort`](crates/legion-protocol/src/lib.rs:1835), [`EditorPort`](crates/legion-protocol/src/lib.rs:1841), [`ProposalPort`](crates/legion-protocol/src/lib.rs:1847), [`EventSinkPort`](crates/legion-protocol/src/lib.rs:1871), and [`StorageRepositoryPort`](crates/legion-protocol/src/lib.rs:1877). Search evidence and implementation review show concrete app code invokes [`EditorEngine`](crates/legion-editor/src/lib.rs:1), [`WorkspaceActor`](crates/legion-project/src/lib.rs:1), and storage repositories directly. Only workspace, security, and observability have meaningful port implementations; storage and editor do not implement their declared service ports. |
...
| Recommendation | Add adapter implementations for [`EditorPort`](crates/legion-protocol/src/lib.rs:1841) over editor engine operations and [`StorageRepositoryPort`](crates/legion-protocol/src/lib.rs:1877) over storage repositories. Add a real proposal service implementing [`ProposalPort`](crates/legion-protocol/src/lib.rs:1847). Refactor application orchestration to depend on port traits or thin domain services instead of directly routing every command to concrete subsystems. |
...
| Evidence | [`AppComposition`](crates/legion-app/src/lib.rs:68) owns concrete workspace and editor instances plus active file and buffer state. [`AppComposition::dispatch_ui_intent()`](crates/legion-app/src/lib.rs:158) directly maps every UI intent to concrete editor/workspace calls. [`AppComposition::active_buffer_projection()`](crates/legion-app/src/lib.rs:247) pulls full text from the editor for UI projection. [`AppComposition::open_workspace()`](crates/legion-app/src/lib.rs:98) hardcodes a correlation id at [`CorrelationId`](crates/legion-app/src/lib.rs:106). |
...
| Evidence | [`AppComposition::open_file()`](crates/legion-app/src/lib.rs:121) resolves a file and then calls workspace read, but uses [`Result::unwrap_or_default()`](crates/legion-app/src/lib.rs:129) for the read result. |
...
| Evidence | [`EventEnvelope`](crates/legion-protocol/src/lib.rs:1725), [`EventSinkPort`](crates/legion-protocol/src/lib.rs:1871), [`InMemoryEventSink`](crates/legion-observability/src/lib.rs:51), [`RedactingEventSink`](crates/legion-observability/src/lib.rs:110), [`transaction_event()`](crates/legion-observability/src/lib.rs:340), [`save_denied_event()`](crates/legion-observability/src/lib.rs:310), and [`watcher_recovery_event()`](crates/legion-observability/src/lib.rs:373) exist. The app and workspace flows reviewed do not inject or emit through an event sink. Phase six requires proposal and conflict event emission in [`foundational-core-ide-platform-implementation-plan-v0.1.md`](plans/foundational-core-ide-platform-implementation-plan-v0.1.md:160). |
...
| Evidence | [`WorkspaceActor::write_file_text()`](crates/legion-project/src/lib.rs:1083) uses the platform service for writes but then calls direct filesystem metadata through [`std::fs::metadata`](crates/legion-project/src/lib.rs:1107). The project crate also duplicates platform error conversion through [`platform_error_from_io()`](crates/legion-project/src/lib.rs:56). Storage metadata APIs exist through [`FileMetadataRecord`](crates/legion-storage/src/lib.rs:34), but workspace write updates are not persisted through storage. |
...
| Evidence | [`PathPolicy::max_write_bytes`](crates/legion-security/src/lib.rs:59) exists, and [`FileWritePolicy`](crates/legion-security/src/lib.rs:326) declares write controls, but [`WorkspaceActor::write_file_text()`](crates/legion-project/src/lib.rs:1083) passes only capability and target path into the broker. [`DenyByDefaultBroker::decide_with_context()`](crates/legion-security/src/lib.rs:528) broadly allows non-write filesystem capabilities at [`DenyByDefaultBroker::decide_with_context()`](crates/legion-security/src/lib.rs:585), LSP variants at [`DenyByDefaultBroker::decide_with_context()`](crates/legion-security/src/lib.rs:603), and network variants at [`DenyByDefaultBroker::decide_with_context()`](crates/legion-security/src/lib.rs:609). |
...
| Evidence | [`DEFAULT_FULL_CACHE_BYTE_BUDGET_BYTES`](crates/legion-text/src/lib.rs:17) caps full-cache operations at five megabytes. [`TextBuffer::try_replace_range()`](crates/legion-text/src/lib.rs:820) validates the post-edit cache size and refreshes cache and line index after each edit at [`TextBuffer::try_replace_range()`](crates/legion-text/src/lib.rs:833). The accepted stress baseline records that 100MB editing triggers full-cache budget failure in [`text-index-stress-baseline.md`](plans/evidence/phase-0/text-index-stress-baseline.md:31), with concrete failure output in [`editor-performance-suite.txt`](plans/evidence/phase-0/editor-performance-suite.txt:19). |
...
| Recommendation | Expose undo history degradation as editor state, emit a retention event through [`EventSinkPort`](crates/legion-protocol/src/lib.rs:1871), and make performance tests configure intentional retention profiles. Add UI projection fields for reduced history availability once tab/session UI expands. |
...
| Evidence | [`SessionRecord`](crates/legion-storage/src/lib.rs:43) persists only workspace id, workspace path, and trust state. [`StorageRepositoryRequest`](crates/legion-protocol/src/lib.rs:1802) covers workspace config and file metadata, but not session restore or proposal audit history. [`StorageRepositoryPort`](crates/legion-protocol/src/lib.rs:1877) exists but storage implementations expose custom repository traits rather than that port. Phase five requires open tabs, active buffer, layout, explorer expansion, panel state, and last workspace persistence in [`foundational-core-ide-platform-implementation-plan-v0.1.md`](plans/foundational-core-ide-platform-implementation-plan-v0.1.md:142). |
...
| Recommendation | Extend protocol storage requests with session restore and proposal audit records. Implement [`StorageRepositoryPort`](crates/legion-protocol/src/lib.rs:1877) for storage backends. Expand [`SessionRecord`](crates/legion-storage/src/lib.rs:43) or add versioned session DTOs for tabs, active buffer, layout, explorer expansion, panel state, and last workspace. |
...
| Evidence | [`Shell`](crates/legion-ui/src/ui.rs:228) no longer owns an editor session and emits [`CommandDispatchIntent`](crates/legion-ui/src/ui.rs:141). However, UI imports [`TextPosition`](crates/legion-ui/src/ui.rs:3) and [`TextRange`](crates/legion-ui/src/ui.rs:3) through the editor crate, and [`ActiveBufferProjection`](crates/legion-ui/src/ui.rs:84) contains full projected text. |
...
| Evidence | [`NativeFileSystem`](crates/legion-platform/src/lib.rs:334) and related services keep platform scope OS-only. [`NativePtyService`](crates/legion-platform/src/lib.rs:346) is stubbed for the milestone. [`shell_title()`](crates/legion-platform/src/lib.rs:603) retains spike labeling. The accepted platform proof states platform remains OS-bound in [`platform-boundary-api-map.md`](plans/evidence/phase-0/platform-boundary-api-map.md:1). |
...
| Evidence | The workspace includes many runtime-surface crates in [`Cargo.toml`](Cargo.toml:3). Later accepted evidence activates `legion-index`, `legion-agent`, `legion-tracker`, `legion-memory`, `legion-plugin`, `legion-collaboration`, `legion-remote`, `legion-remote-transport`, `legion-terminal`, `legion-telemetry`, and `legion-retention` only within bounded policy surfaces described in [`plans/dependency-policy.md`](plans/dependency-policy.md:769). |
...
| Evidence | [`ModelProvider`](crates/legion-ai/src/lib.rs:215) is defined in the AI crate, and provider adapters depend inward through [`make_stub_registry()`](crates/legion-ai-providers/src/lib.rs:16). This preserves the provider inversion noted in [`architecture-review-v0.1.md`](plans/architecture-review-v0.1.md:11). |
...
| Recommendation | Keep AI isolated until editor/workspace proposal and observability paths are complete. When AI becomes active, require all generated edits to enter through [`WorkspaceProposal`](crates/legion-protocol/src/lib.rs:783) and all provider calls to emit redacted events through [`EventSinkPort`](crates/legion-protocol/src/lib.rs:1871). |

## Positive architectural findings

| Area | Evidence | Assessment |
|---|---|---|
| UI ownership | [`Shell`](crates/legion-ui/src/ui.rs:228) and [`CommandDispatchIntent`](crates/legion-ui/src/ui.rs:141) | The current shell no longer owns editor session state and emits typed command intents. |
...
| Editor transactions | [`TransactionRecord::to_protocol_descriptor()`](crates/legion-editor/src/lib.rs:109) and [`cargo-test-workspace-all-targets.txt`](plans/evidence/phase-0/cargo-test-workspace-all-targets.txt:48) | Editor records pre/post snapshots, transaction metadata, and deterministic ordering for covered flows. |
| Path boundary tests | [`path_boundary.rs`](crates/legion-project/tests/path_boundary.rs:1) and [`cargo-test-workspace-all-targets.txt`](plans/evidence/phase-0/cargo-test-workspace-all-targets.txt:128) | Parent escape, Windows path cases, and symlink escape behaviors are covered. |
| Observability primitives | [`RedactingEventSink`](crates/legion-observability/src/lib.rs:110) and [`cargo-test-workspace-all-targets.txt`](plans/evidence/phase-0/cargo-test-workspace-all-targets.txt:94) | Event schema validation and metadata-only redaction are implemented and tested. |
| AI provider inversion | [`ModelProvider`](crates/legion-ai/src/lib.rs:215) and [`make_stub_registry()`](crates/legion-ai-providers/src/lib.rs:16) | Core AI abstractions do not depend on provider adapters. |

## Refactoring sequence

> Rebaseline note: step 1 below is historically resolved for the current manual save flow via [`SaveWorkflowService::save_active_buffer()`](crates/legion-app/src/lib.rs:938). Keep it as a template for broadening proposal mediation, conflict handling, and auditability to non-save mutation sources.

1. **Close durable mutation safety first.** Implement proposal-mediated save, expected fingerprint checks, typed conflict states, fail-closed atomic fallback, and save/proposal tests around [`AppComposition::save_active_buffer()`](crates/legion-app/src/lib.rs:214), [`WorkspaceActor::write_file_text()`](crates/legion-project/src/lib.rs:1083), [`WorkspaceProposal`](crates/legion-protocol/src/lib.rs:783), and [`FileConflictState`](crates/legion-protocol/src/lib.rs:502).
2. **Introduce real port adapters.** Implement [`EditorPort`](crates/legion-protocol/src/lib.rs:1841), [`ProposalPort`](crates/legion-protocol/src/lib.rs:1847), and [`StorageRepositoryPort`](crates/legion-protocol/src/lib.rs:1877), then narrow application orchestration to command routing and lifecycle wiring.
...
| Large-file degraded mode | Open a file above [`DEFAULT_FULL_CACHE_BYTE_BUDGET_BYTES`](crates/legion-text/src/lib.rs:17) through a degraded path and prove viewport edit latency is bounded. |
