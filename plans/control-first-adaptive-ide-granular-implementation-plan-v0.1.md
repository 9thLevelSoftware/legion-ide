...
The research establishes that developers want AI leverage without agency loss: fast manual editing, high-quality semantic assistance, reviewable delegated work, and governable autonomy. The finalized design maps that thesis to existing Legion IDE strengths: proposal-mediated saves through [`SaveWorkflowService`](crates/legion-app/src/lib.rs:935), workspace write authority through [`WorkspaceSaveRequest`](crates/legion-project/src/lib.rs:133), projection-only UI through [`ActiveBufferProjection`](crates/legion-ui/src/ui.rs:86), deterministic editor ownership through [`EditorEngine`](crates/legion-editor/src/lib.rs:340), protocol boundaries through [`WorkspaceProposal`](crates/legion-protocol/src/lib.rs:1472), and metadata-only observability through [`proposal_audit_record()`](crates/legion-observability/src/lib.rs:394).
...
5. [`dependency-policy.md`](plans/dependency-policy.md:88) limits [`legion-index`](crates/legion-index/src/lib.rs:1) to [`legion-protocol`](crates/legion-protocol/src/lib.rs:1), [`legion-storage`](crates/legion-storage/src/lib.rs:1), and [`legion-text`](crates/legion-text/src/lib.rs:1) during Phase 3 activation.
...
| Proposal execution | Yes | [`AppProposalCoordinator`](crates/legion-app/src/lib.rs:354), [`ProposalRequest`](crates/legion-protocol/src/lib.rs:3727), [`ProposalResponse`](crates/legion-protocol/src/lib.rs:3746) | LSP apply, AI edits, plugin writes, remote writes, collaboration writes | Phase 2 evidence updated with apply, rollback, denial, audit tests |
| Proposal UI projection | Yes, DTO and projection only | [`Shell`](crates/legion-ui/src/ui.rs:239), [`CommandDispatchIntent`](crates/legion-ui/src/ui.rs:154) | UI-side apply or editor state ownership | UI emits intents only |
| Viewport non-regression | Yes | [`ActiveBufferProjection`](crates/legion-ui/src/ui.rs:86), [`TextSnapshot`](crates/legion-text/src/lib.rs:240) | Full-source UI projection regressions | Large files remain viewport-only |
| Semantic boundary remediation | Accepted Phase 3 scope | `RepositoryDiscoveryImporter`, descriptor/lease source inputs, syntax-cache freshness, metadata storage | AI/plugin/remote/collaboration activation | No production disk scan or unbounded full-source persistence in [`legion-index`](crates/legion-index/src/lib.rs:1) |
| Semantic runtime | After semantic boundary remediation | [`IndexingActor`](crates/legion-index/src/lib.rs:253) | AI activation | Bounded queues, cancellation, no input or save blocking |
...
| Trust surfaces | DTO and projection now | [`event_metadata_record()`](crates/legion-observability/src/lib.rs:376), [`InMemoryStorageRepositoryPort`](crates/legion-storage/src/lib.rs:209) | Raw source or prompt persistence | Metadata-only context and privacy records |
| Assisted AI | Planning now, runtime later | [`ModelProvider`](crates/legion-ai/src/lib.rs:216), semantic queries, proposal service | Direct AI mutation or hidden egress | AI outputs proposals or explanations only |
| Delegated agents | ADR and schema planning only | [`legion-agent`](crates/legion-agent/src/lib.rs:1), [`legion-tracker`](crates/legion-tracker/src/lib.rs:1) | Self-approving or direct mutation | Task state machine with proposal-only writes |
...
- [`legion-protocol`](crates/legion-protocol/src/lib.rs:1) remains the only cross-domain DTO and ID boundary.
- [`legion-ui`](crates/legion-ui/src/lib.rs:1) must not gain dependencies on [`legion-editor`](crates/legion-editor/src/lib.rs:1), [`legion-project`](crates/legion-project/src/lib.rs:1), or [`legion-storage`](crates/legion-storage/src/lib.rs:1).
...
Objectives:
- Audit [`legion-app`](crates/legion-app/src/lib.rs:1), [`legion-project`](crates/legion-project/src/lib.rs:1), [`legion-editor`](crates/legion-editor/src/lib.rs:1), and [`legion-ui`](crates/legion-ui/src/lib.rs:1) for mutation entry points.
...
Architectural constraints:
- Save path must remain [`AppComposition::save_active_buffer()`](crates/legion-app/src/lib.rs:1716) to [`SaveWorkflowService`](crates/legion-app/src/lib.rs:935) to [`WorkspaceActor::save_file_with_proposal()`](crates/legion-project/src/lib.rs:2477).
...
Acceptance criteria:
- Existing test [`workspace_vfs_integration_external_overwrite_between_open_and_save_yields_conflict`](crates/legion-app/tests/workspace_vfs_integration.rs:1) remains green.
...
Objectives:
- Harden [`AppProposalCoordinator`](crates/legion-app/src/lib.rs:354) as the app-owned proposal lifecycle coordinator unless an ADR approves a new crate.
- Enforce legal transitions for [`ProposalLifecycleState`](crates/legion-protocol/src/lib.rs:1969) across [`ProposalRequest`](crates/legion-protocol/src/lib.rs:3727) variants.
- Preserve diagnostics for missing context, invalid transition, stale proposal, expired proposal, and denied validation.

Architectural constraints:
- Lifecycle state records must carry non-zero [`CorrelationId`](crates/legion-protocol/src/lib.rs:62) and non-nil [`CausalityId`](crates/legion-protocol/src/lib.rs:82).
...
- Invalid direct Created to Applied transition returns [`ProposalResponse`](crates/legion-protocol/src/lib.rs:3746) with diagnostics and no mutation.
...
Objectives:
- Make target discovery total for [`ProposalPayload`](crates/legion-protocol/src/lib.rs:1507), including nested [`BatchProposalPayload`](crates/legion-protocol/src/lib.rs:1691).
...
- File validations must use [`WorkspaceFileIdentity`](crates/legion-protocol/src/lib.rs:218) and workspace authority rather than path strings alone.
...
Acceptance criteria:
- Unit tests cover each [`ProposalPayload`](crates/legion-protocol/src/lib.rs:1507) variant.
...
Objectives:
- Route open-buffer text edits through [`EditorEngine`](crates/legion-editor/src/lib.rs:340) transactions rather than direct text replacement in UI or workspace.
...
Objectives:
- Route create, delete, rename, save, and workspace edit payloads through [`WorkspaceActor`](crates/legion-project/src/lib.rs:1158) authority.
- Use request shapes consistent with [`WorkspaceSaveRequest`](crates/legion-project/src/lib.rs:133) and existing create, delete, and rename request structs.
...
- Closed-file apply may not bypass security policy in [`legion-security`](crates/legion-security/src/lib.rs:1).
...
- Path boundary tests in [`path_boundary.rs`](crates/legion-project/tests/path_boundary.rs:1) remain green.
...
Objectives:
- Complete [`BatchPreflightPlan`](crates/legion-app/src/lib.rs:265), [`BatchExecutionContract`](crates/legion-app/src/lib.rs:298), and [`ProposalBatchRollbackPolicy`](crates/legion-protocol/src/lib.rs:1532) behavior.
...
Objectives:
- Add projection DTOs in [`legion-protocol`](crates/legion-protocol/src/lib.rs:1) for proposal ledger rows, context manifest summaries, risk labels, privacy labels, and diff summaries.
- Extend [`ShellProjectionSnapshot`](crates/legion-ui/src/ui.rs:126) or a sibling projection type to display proposal state without owning it.
- Add [`CommandDispatchIntent`](crates/legion-ui/src/ui.rs:154) variants for approve, reject, preview, rollback, cancel, and open proposal details.

Architectural constraints:
- UI may emit intents only; app resolves intents to [`ProposalRequest`](crates/legion-protocol/src/lib.rs:3727).
...
- Ensure [`event_metadata_record()`](crates/legion-observability/src/lib.rs:376) and [`proposal_audit_record()`](crates/legion-observability/src/lib.rs:394) are used consistently.
- Persist audit summaries through [`InMemoryStorageRepositoryPort`](crates/legion-storage/src/lib.rs:209) or an approved port implementation.
...
- Existing redaction tests in [`legion-observability`](crates/legion-observability/src/lib.rs:1) remain green.
...
Objectives:
- Add tests around [`ActiveBufferProjection`](crates/legion-ui/src/ui.rs:86), [`TextLineSlice`](crates/legion-text/src/lib.rs:215), [`TextChunkDescriptor`](crates/legion-text/src/lib.rs:151), and [`TextSnapshot`](crates/legion-text/src/lib.rs:240).
...
- [`performance_suite.rs`](crates/legion-editor/tests/performance_suite.rs:1) remains usable as a perf listing and regression signal.
- Small-buffer preview still works within the existing 5 MiB full-cache budget in [`legion-text`](crates/legion-text/src/lib.rs:11).
...
Architectural constraints:
- [`EditorEngine`](crates/legion-editor/src/lib.rs:340) remains text owner.
- [`TextSnapshot::try_full_text()`](crates/legion-text/src/lib.rs:349) remains bounded and non-default for large files.
...
- No new crate depends on [`legion-editor`](crates/legion-editor/src/lib.rs:1) for read access unless an ADR explicitly allows it.
...
Objectives:
- Replace production use of direct filesystem scanning in [`RepositoryScanner::scan()`](crates/legion-index/src/lib.rs:787) with workspace-authored discovery DTOs from [`legion-project`](crates/legion-project/src/lib.rs:1) through [`legion-protocol`](crates/legion-protocol/src/lib.rs:1).
...
Architectural constraints:
- [`legion-index`](crates/legion-index/src/lib.rs:1) must not become a workspace authority.
- Path boundary and trust decisions stay in [`legion-project`](crates/legion-project/src/lib.rs:1) and [`legion-security`](crates/legion-security/src/lib.rs:1).

Acceptance criteria:
- Production index flows accept workspace discovery deltas rather than arbitrary root paths.
- Dependency policy remains green and forbids [`legion-index`](crates/legion-index/src/lib.rs:1) from depending on [`legion-project`](crates/legion-project/src/lib.rs:1).
...
Objectives:
- Refactor [`SourceDocument`](crates/legion-index/src/lib.rs:937) to avoid mandatory full source ownership.
...
Acceptance criteria:
- Large-file semantic tests do not call [`TextSnapshot::try_full_text()`](crates/legion-text/src/lib.rs:349) as the default path.
...
Objectives:
- Extend [`SyntaxCacheKey`](crates/legion-index/src/lib.rs:1066) beyond content hash, language id, and grammar version to include privacy scope, workspace generation, schema version, and parser or model version where applicable.
...
Objectives:
- Define semantic record storage through [`legion-storage`](crates/legion-storage/src/lib.rs:1) using file identity, ranges, hashes, symbol ids, relation ids, and timestamps.
...
Objectives:
- Activate [`IndexingActor`](crates/legion-index/src/lib.rs:253) as the owner of indexing queues, priorities, cancellation, and backpressure.
...
- Route edit-producing LSP responses into [`WorkspaceProposal`](crates/legion-protocol/src/lib.rs:1472).
...
Architectural constraints:
- DTOs live in [`legion-protocol`](crates/legion-protocol/src/lib.rs:1).
- Rendering lives in [`legion-ui`](crates/legion-ui/src/ui.rs:1) as snapshots and intents.
- Policy resolution belongs in app or a future approved policy service, not UI.

Acceptance criteria:
- Contract tests prove DTO serialization stability in [`dto_contracts.rs`](crates/legion-protocol/tests/dto_contracts.rs:1).
...
Objectives:
- Expand [`ModelProvider`](crates/legion-ai/src/lib.rs:216) integration into app-level orchestration without letting providers mutate workspace state.
...
Architectural constraints:
- Provider output that edits files becomes [`WorkspaceProposal`](crates/legion-protocol/src/lib.rs:1472).
...
Requires ADR, policy update, and contract tests before activating [`legion-tracker`](crates/legion-tracker/src/lib.rs:1).
...
Objectives:
- Activate [`legion-agent`](crates/legion-agent/src/lib.rs:1) only as an orchestrator over context manifests, providers, semantic queries, trackers, and proposals.
...
- Draft terminal ADR: commands are proposals with sandbox, cwd, environment, output retention, and approval policy. Use [`ProposalPayload::TerminalCommand`](crates/legion-protocol/src/lib.rs:1507) as DTO seed only.
...
- No future runtime bypasses [`WorkspaceProposal`](crates/legion-protocol/src/lib.rs:1472), projection-only UI, metadata-only observability, or ADR-0015 large-file rules.
...
cargo test -p legion-editor --test performance_suite -- --list
cargo test -p legion-protocol --test dto_contracts
cargo test -p legion-project --test path_boundary
cargo test -p legion-project --test watcher_recovery
```
...
- Any non-user-direct mutation path applies edits without [`WorkspaceProposal`](crates/legion-protocol/src/lib.rs:1472).
