...
| Phase 2 — Proposal mutation substrate | **Partially accepted** | [`plans/evidence/phase-2/proposal-mutation-substrate.md`](evidence/phase-2/proposal-mutation-substrate.md) | DTOs, routing, batch structures, lifecycle states, and deny-by-default validation are accepted. Runtime apply beyond saves remains denied until Phase 2B completes. |
...
| ADR-0016 — Generalized proposal service | Accepted | Runtime apply beyond saves is the Phase 2B implementation gap, not an ADR gap. |
...
- [`plans/architecture-review-full-codebase-v0.1.md`](architecture-review-full-codebase-v0.1.md) Finding 1 (save bypasses proposal) — **Historical**. Manual saves now route through `SaveWorkflowService::save_active_buffer()` and `WorkspaceActor::save_file_with_proposal()`. The remaining gap is generalized proposal execution for non-save mutation classes (Phase 2B), not a missing save pipeline.
...
- [`plans/architecture-review-full-codebase-v0.1.md`](architecture-review-full-codebase-v0.1.md) mermaid diagram labeled "missing" for Proposal/Observability/Storage — **Historical**. Proposal lifecycle, observability event emission, and storage audit persistence are now wired for the manual save path. The remaining gap is universal mediation across all mutation classes.
