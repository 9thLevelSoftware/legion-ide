...
- OS accessibility-tree inspection, **macOS**: committed probe `scripts/a11y-ax-walk.sh`. Live dump when run on Darwin against a window. The 2026-08-16 WS18-T2 dump remains an unreproducible external observation. Not VoiceOver.
- OS accessibility-tree inspection, **Linux**: committed probe `scripts/a11y-atspi-walk.sh`. Live dump when run on Linux against a window. Not Orca.
...
- Linux OS accessibility tree: probe committed (`scripts/a11y-atspi-walk.sh`). A live dump still has to be captured on Linux. Not Orca.
- macOS reproducibility: probe committed (`scripts/a11y-ax-walk.sh`). The 2026-08-16 WS18-T2 dump is still not produced by that script. Not VoiceOver.
