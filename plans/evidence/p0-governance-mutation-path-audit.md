...
- Durable saves remain proposal-mediated through [`AppComposition::save_active_buffer()`](../../crates/devil-app/src/lib.rs), [`SaveWorkflowService`](../../crates/devil-app/src/lib.rs), and [`WorkspaceActor::save_file_with_proposal()`](../../crates/devil-project/src/lib.rs).
- UI remains projection-only: [`devil-ui`](../../crates/devil-ui/src/lib.rs) consumes protocol projections, emits [`CommandDispatchIntent`](../../crates/devil-ui/src/ui.rs), and has no direct dependency on editor, project, storage, or app crates.
...
The current save-path regression suite in [`workspace_vfs_integration.rs`](../../crates/devil-app/tests/workspace_vfs_integration.rs) covers:
...
- Dependency policy now records [`devil-ui`](../../crates/devil-ui/Cargo.toml) as protocol-only and explicitly forbids `devil-ui -> devil-app`, `devil-ui -> devil-editor`, `devil-ui -> devil-project`, and `devil-ui -> devil-storage`.
