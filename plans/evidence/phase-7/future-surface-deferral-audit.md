...
- Standalone `legion-terminal`, `legion-lsp`, and production remote transport crates remain future-gated outside the accepted deterministic Phase 7 harness.
...
- `legion-remote` is active only for deterministic edge workspace harness behavior, protocol DTO validation, policy-gated descriptors, reconnect/offline metadata, and metadata-only audit.
