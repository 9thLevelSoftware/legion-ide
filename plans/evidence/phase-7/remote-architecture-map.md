...
- Dependency policy: `plans/dependency-policy.md` activates the limited Phase 7 `legion-remote` boundary and required Phase 7 protocol DTO symbols.
- Contract tests: `cargo test -p legion-protocol --test dto_contracts` covers remote DTO serialization and fail-closed validation helpers.
...
- `legion-protocol` owns remote identity, lifecycle, transport envelope, filesystem snapshot, filesystem operation, write precondition, process, PTY, LSP, semantic query, operation checkpoint, offline resume, and metadata-only audit DTOs.
- `legion-remote` owns deterministic remote session lifecycle, fixture filesystem metadata, proposal-gated remote fixture mutation, bounded descriptor validation, reconnect/offline state, and metadata-only remote audit summaries.
- `legion-security` owns remote capability policy and denies remote sessions, filesystem, execution, LSP, semantic query, egress, audit, and offline resume by default.
- `legion-observability` and `legion-storage` own metadata-only remote event and storage records.
- `legion-app` owns remote session composition, explicit runtime enablement, remote transport dispatch, and audit persistence.
- `legion-ui` consumes projections and emits intents only; no remote ownership is introduced.
...
- Runtime lifecycle, filesystem, execution, reconnect, offline resume, and audit: `cargo test -p legion-remote`.
- App-owned composition and local disk preservation: `cargo test -p legion-app --test workspace_vfs_integration remote`.
- Security capabilities: `cargo test -p legion-security remote`.
- Storage/observability redaction: `cargo test -p legion-storage remote_audit` and `cargo test -p legion-observability remote_audit`.
...
- [x] `legion-remote` exists only after approved ADR, policy, protocol, tests, and evidence gates.
