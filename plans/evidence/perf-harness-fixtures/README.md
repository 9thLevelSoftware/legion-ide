...
- `100k-file-search.toml`: intended bounded search over a 100K-file fixture
  corpus. Not wired, and there is no 100K-file workload in the harness at all.
  The generation command that `plans/evidence/production/WS-MANUAL-02/reference-workspaces.md`
  cites for the 100K-file reference workspace, `xtask generate-test-workspace`,
  does not exist as an `xtask` subcommand.

Wiring these is the remaining work for `P8.F4.T1`. Until then, do not cite this
directory as evidence that a large-fixture benchmark runs.
