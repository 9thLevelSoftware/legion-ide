...
| Item | Location |
| --- | --- |
| Adapter resolution | `crates/legion-debug/src/adapter_resolve.rs` (`LEGION_DAP_MODE`, `LEGION_DAP_ADAPTER`, PATH, `LEGION_DAP_USE_FAKE`) |
| Trust deny | `DebugWorkflow::launch` — untrusted → `debug.adapter.launch denied` |
| Live one-shot launch | App uses `LiveDapSession` when adapter resolves; else fixture |
...
| Variable | Meaning |
| --- | --- |
| `LEGION_DAP_MODE` | `fixture` \| `live` \| `auto` (default) |
| `LEGION_DAP_ADAPTER` | Absolute path to adapter executable |
