...
- `crates/legion-app/tests/control_trust_surfaces.rs` covers preview, approve, reject, cancel, apply, rollback, stale, conflict, failed, and rolled-back lifecycle visibility through app-owned proposal authority and shell projections.
...
- `cargo test -p legion-app --test control_trust_surfaces proposal_states_visible -- --nocapture` passed: 1 test passed.
- `cargo test -p legion-app --test control_trust_surfaces dirty_text_preserved -- --nocapture` passed: 1 test passed.
- `cargo test -p legion-app --test control_trust_surfaces proposal_details -- --nocapture` passed: 1 test passed.
- `cargo test -p legion-app --test control_trust_surfaces -- --nocapture` passed: 7 tests passed.
...
- `cargo test -p legion-app --test control_trust_surfaces assisted_ai_propose_is_proposal_only -- --nocapture` passed: 1 test passed.
- `cargo test -p legion-app --test control_trust_surfaces assisted_ai -- --nocapture` passed: 3 tests passed.
- `cargo test -p legion-desktop --test control_trust_view -- --nocapture` passed: 3 tests passed.
...
- `cargo test -p legion-desktop --test control_trust_view -- --nocapture` passed: 3 tests passed.
- `cargo test -p legion-ui control_trust -- --nocapture` passed: 2 tests passed.
...
- `legion-ui` tests verify control/trust shell projections are static and command parsing only emits `CommandDispatchIntent`.
- `legion-desktop` bridge tests verify proposal and AI controls translate into intents using projected ids and display-safe labels.
...
- `cargo test -p legion-desktop --test control_trust_bridge -- --nocapture` passed: 5 tests passed.
- `cargo test -p legion-ui control_trust -- --nocapture` passed: 2 tests passed.
...
- The exact `cargo test -p legion-desktop --test projection_rendering control -- --nocapture` command from Plan 05-05 matches no tests in that target; the unfiltered projection rendering target passed separately in 05-05.
