...
- `devil-ui` remains projection-only and owns no editor text, editor sessions, terminal runtime, workspace actor, or disk mutation path.
- `devil-app` composes the activated language and terminal workflows.
...
- Dependency policy explicitly authorizes GUI Phase 4 `devil-app -> devil-index` and `devil-app -> devil-terminal` edges only.
- `crates/devil-ui/src/ui.rs` only emits `CommandDispatchIntent` variants and consumes `LanguageToolingProjection` / `TerminalPanelProjection`.
- `crates/devil-app/src/lib.rs` owns `LanguageToolingWorkflow` and `TerminalWorkflow`.
