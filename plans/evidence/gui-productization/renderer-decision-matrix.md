...
- Renderer dependencies stay out of `legion-ui` and core crates.
...
Phase 2 should implement the first renderer-backed desktop proof as a new `legion-desktop` adapter using `eframe`/`egui` plus the framework-provided native integration stack. This does not move `legion-ui` or core crates to egui. `legion-ui` remains projection-only, and `legion-desktop` consumes `ShellProjectionSnapshot` and emits `CommandDispatchIntent` through `legion-app`.
...
- The adapter requires `legion-ui` or core crates to depend on renderer/windowing crates.
...
- Add `legion-desktop` as the only crate allowed to depend on `eframe`, `egui`, and renderer/windowing integration crates.
