...
- No `devil-index` dependency on a vector database, embedding library, AI provider, model provider, or retrieval crate exists.
- No `devil-index` code path computes embeddings or stores vector records.
...
- `devil-agent`, `devil-memory`, `devil-tracker`, AI provider, plugin, remote, terminal, and collaboration runtime surfaces remain outside accepted Phase 3 activation.
