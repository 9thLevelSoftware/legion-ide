...
Phase 2 of [`plans/implementation-plan.md`](../implementation-plan.md:117) promotes the current save-specific proposal path into a universal mutation substrate. The current save path is the safety baseline: [`SaveProposalCoordinator`](../../crates/legion-app/src/lib.rs:120) builds save proposals, [`SaveWorkflowService::save_active_buffer()`](../../crates/legion-app/src/lib.rs:973) preserves dirty editor text on rejected outcomes, and [`WorkspaceActor::save_file_with_proposal()`](../../crates/legion-project/src/lib.rs:1622) enforces fingerprint, version, capability, correlation, causality, and fail-closed write semantics.

The existing contract in [`WorkspaceProposal`](../../crates/legion-protocol/src/lib.rs:1343) and [`ProposalPayload`](../../crates/legion-protocol/src/lib.rs:1378) is close enough for manual saves but incomplete for multi-file edits, future LSP/code actions, plugin actions, terminal commands, AI patches, collaboration operations, and remote workspace mutations. A save-only assumption remains in [`proposal_file_identity()`](../../crates/legion-app/src/lib.rs:1357), which will be replaced by later Phase 2 app orchestration work rather than by this protocol-only subtask.

UI remains projection-only. [`Shell`](../../crates/legion-ui/src/ui.rs:228) may render proposal state and enqueue command intents, but it must not own proposal execution, editor sessions, text mutation, workspace VFS state, or durable writes.

## Decision

Adopt a generalized proposal service contract centered on [`WorkspaceProposal`](../../crates/legion-protocol/src/lib.rs:1343), [`ProposalRequest`](../../crates/legion-protocol/src/lib.rs:2804), and [`ProposalResponse`](../../crates/legion-protocol/src/lib.rs:2823). The contract is implemented first as protocol DTOs and contract tests; later app-domain orchestration replaces save-specific coordination without weakening save guarantees.
...
- Lifecycle intents are explicit request DTOs. [`ProposalRequest`](../../crates/legion-protocol/src/lib.rs:2804) continues to support validate, preview, and apply, and adds approve, reject, cancel, and rollback commands through [`ProposalLifecycleCommand`](../../crates/legion-protocol/src/lib.rs:2007).
...
- Multi-target work is represented by [`BatchProposalPayload`](../../crates/legion-protocol/src/lib.rs:1499), embedded as [`ProposalPayload::Batch`](../../crates/legion-protocol/src/lib.rs:1396).
- Batch item order is deterministic. [`ProposalBatchItem::order`](../../crates/legion-protocol/src/lib.rs:1526) is the authoritative apply order, and dependency edges are explicit metadata.
- Atomicity boundaries are explicit via [`ProposalBatchAtomicity`](../../crates/legion-protocol/src/lib.rs:1401): all-or-nothing, prepare-all-before-mutate, or ordered non-atomic with mandatory partial-failure records.
...
- Rollback policy is explicit via [`ProposalBatchRollbackPolicy`](../../crates/legion-protocol/src/lib.rs:1412): required, best-effort, not-supported, or not-required.
- Rollback plans use deterministic [`ProposalRollbackStep`](../../crates/legion-protocol/src/lib.rs:1584) records and typed [`ProposalRollbackAction`](../../crates/legion-protocol/src/lib.rs:1563) values.
...
- If rollback cannot restore exact prior state, the service must emit a failed rollback outcome with [`ProposalPartialFailureRecord`](../../crates/legion-protocol/src/lib.rs:1620) metadata and must preserve user buffers rather than discarding dirty editor text.
...
- Observability constraints remain mandatory: non-zero [`CorrelationId`](../../crates/legion-protocol/src/lib.rs:73), non-nil [`CausalityId`](../../crates/legion-protocol/src/lib.rs:201), and non-zero [`EventSequence`](../../crates/legion-protocol/src/lib.rs:93).
...
- This ADR does not activate [`crates/legion-agent/src/lib.rs`](../../crates/legion-agent/src/lib.rs:1), [`crates/legion-index/src/lib.rs`](../../crates/legion-index/src/lib.rs:1), [`crates/legion-memory/src/lib.rs`](../../crates/legion-memory/src/lib.rs:1), or [`crates/legion-tracker/src/lib.rs`](../../crates/legion-tracker/src/lib.rs:1).
- This ADR does not replace [`SaveProposalCoordinator`](../../crates/legion-app/src/lib.rs:120), implement generalized apply orchestration, or remove [`proposal_file_identity()`](../../crates/legion-app/src/lib.rs:1357) in the protocol-contract subtask.
