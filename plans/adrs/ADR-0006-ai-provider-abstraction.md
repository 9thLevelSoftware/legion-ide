...
- `devil-ai-providers` exposes concrete local adapters for deterministic local tests, Ollama loopback, and llama.cpp loopback.
- Ollama uses the native local HTTP API and `OLLAMA_BASE_URL`, defaulting to `http://localhost:11434`.
- llama.cpp uses the server's OpenAI-compatible `/v1/chat/completions` and `/v1/embeddings` routes through `LEGION_LLAMA_CPP_BASE_URL`, legacy `DEVIL_LLAMA_CPP_BASE_URL`, or `LLAMA_CPP_BASE_URL`, defaulting to `http://localhost:8080/v1`. `LEGION_LLAMA_CPP_API_KEY`, legacy `DEVIL_LLAMA_CPP_API_KEY`, or `LLAMA_CPP_API_KEY` may attach an optional local bearer token; no token is required by default.
- Generic OpenAI-compatible BYOK providers remain separate from llama.cpp and require `LEGION_OPENAI_COMPATIBLE_API_KEY`, legacy `DEVIL_OPENAI_COMPATIBLE_API_KEY`, or `OPENAI_API_KEY` before network invocation.
