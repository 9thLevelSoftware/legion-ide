...
## Context
Devil IDE runs multiple long-lived subsystems (indexing, AI orchestration, file watching, tracker) that must not block the editor core or UI thread. Shared mutable state across these subsystems would violate Rust's safety guarantees and architectural clarity.
