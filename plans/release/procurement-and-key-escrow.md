...
| --- | --- | --- | --- | --- | --- |
| Apple Developer Program (organization or individual) | `EXT-CERT-MAC` — Developer ID signing + notarization (Phase 5); fresh-VM Gatekeeper evidence (P8.F1.T3) | $99/yr | Days (identity verification) | developer.apple.com/programs | ☑ **acquired 2026-08-17** (owner-confirmed; certificates not yet issued or wired into CI). Queue: [#211](https://github.com/9thLevelSoftware/legion-ide/issues/211) |
| Azure Trusted Signing account + identity validation | `EXT-CERT-WIN` — Authenticode signing for MSI via signtool dlib; SmartScreen reputation accrual starts at first signed release | ~$9.99/mo (Basic) | 1–3 weeks (identity validation) | portal.azure.com → Trusted Signing | ☐ Queue: [#213](https://github.com/9thLevelSoftware/legion-ide/issues/213) |
| Linux signing keypair (minisign/Ed25519) | `EXT-CERT-LIN` — detached signatures for DEB/AppImage + signed SHA256SUMS | $0 (generate locally) | Immediate | `minisign -G` (store per escrow policy below) | ☐ Queue: [#212](https://github.com/9thLevelSoftware/legion-ide/issues/212) |
