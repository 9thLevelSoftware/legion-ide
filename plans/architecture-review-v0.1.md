...
1. **`legion-ai` provider boundary inversion**
   - `legion-ai` now defines abstractions and request/response types and does not depend on `legion-ai-providers`.
   - `legion-ai-providers` now depends on `legion-ai` and is an adapter implementation crate.
...
2. **Editor/Project coupling remains protocol-based**
   - No hard dependency from `legion-editor` to `legion-project` will be added.
   - The boundary is defined in `legion-protocol` as data-centric contract types + traits.
...
- **Finding:** Previously inverted dependency between `legion-ai` and `legion-ai-providers`.
- **Evidence:** Previous manifest state in [`crates/legion-ai/Cargo.toml`](crates/legion-ai/Cargo.toml:8-13) and corrected state now
- **Edit:** Completed by dependency inversion in `legion-ai` and `legion-ai-providers` manifests.
...
- **Evidence:** [`§2.2`](plans/architecture-charter-v0.1.md:126-151), previous `crates/legion-editor/Cargo.toml`
- **Edit:** Add explicit project-related contract primitives to `legion-protocol` so editor can consume immutable project context without direct coupling.
...
- **Finding:** `legion-platform` still appears to include editor/window integration concerns.
- **Evidence:** [`§3.2`](plans/architecture-charter-v0.1.md:235-253), [`§1.5`](plans/architecture-charter-v0.1.md:58-65)
- **Edit:** Bound `legion-platform` to OS services and create dedicated platform boundary proof before Spike 1A.
...
2. Add protocol boundary contract for editor/project interaction in `legion-protocol`.
...
7. Keep these crates placeholder-only for Spike 1A: `legion-agent`, `legion-memory`, `legion-ai-providers` (adapters), `legion-cli`, `legion-observability`.
