...
- `cargo run -p devil-app -- <path>` is a minimal interactive proof: it trusts the current directory and only reacts to `:w` and `:q`.
- If `xtask check-deps` fails for protocol symbols, it checks literal `struct`/`enum`/`trait` definitions in `crates/devil-protocol/src/lib.rs`; type aliases or reexports will not satisfy it.
