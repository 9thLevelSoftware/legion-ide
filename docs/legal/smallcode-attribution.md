...
| `src/tools/two_stage_router.js`, `src/session/action_classifier.js` | `crates/legion-ai/src/routing.rs` (`classify_action`, `tools_for_action`) | semantics — query/mutate directive classification and narrowing of the advertised tool set within the scope. Re-implemented in Rust from documented behavior; no source copied. Legion withholds only the edit tool, and only from a query, rather than SmallCode's wider cut. | ported 2026-08-23 |
...
| `src/tools/read_tracker.js`, `src/tools/dedup.js`, `src/tools/trust_decay.js` | `crates/legion-agent/src/governors.rs` (`LoopGovernors`) | semantics — session-scoped read-before-write tracking, pure-tool call dedup keyed on a canonical (tool, arguments) fingerprint, and per-tool consecutive-failure demotion. Re-implemented in Rust from documented behavior; no source copied. Legion's demotion is advisory only: it appends a notice to the failure the model is already shown and withdraws nothing, because the scope decides what a tool may do. | read tracking and dedup already present; demotion ported 2026-08-23 |
| `src/session/plan_tracker.js`, `src/session/dependency_graph.js`, `src/session/contract.js` | `crates/legion-agent/src/plan_anchor.rs` (`PlanAnchor`) | semantics — capture of a model-stated plan and periodic re-injection against multi-turn drift, from `plan_tracker.js` only. Re-implemented in Rust from documented behavior; no source copied. `dependency_graph.js` (file-overlap batching) and `contract.js` (definition-of-done assertions) remain unported. | plan capture and re-injection ported 2026-08-23; the other two still planned |
