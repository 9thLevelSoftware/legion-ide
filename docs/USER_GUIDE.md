...
> **Assist / Delegate provider routing today.** Product composition is **local-first by default** (`Auto`): a fast loopback probe prefers **Ollama** (`OLLAMA_BASE_URL`, default `http://127.0.0.1:11434`, model `OLLAMA_MODEL` or `llama3.2`) when reachable, else **Anthropic** when BYOK credentials exist (env `ANTHROPIC_API_KEY` / `LEGION_ANTHROPIC_API_KEY` or OS keyring via the Model Picker **Save Anthropic key** control), else the offline `deterministic-local` fixture (CI / zero-egress). Override with Model Picker route buttons or `LEGION_AI_PROVIDER=auto|ollama|anthropic|deterministic`. Anthropic uses progressive Messages **SSE**. **Delegate chat** with a live provider runs generation on a **background worker** (UI returns immediately with “Streaming response…”; frame poll finalizes the assistant turn). Assist edit proposals still complete on the calling thread (proposal registration stays synchronous). Results remain proposal-mediated / accept-gated.
