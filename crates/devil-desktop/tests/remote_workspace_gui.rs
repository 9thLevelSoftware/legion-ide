...
impl TempWorkspace {
    fn new() -> Self {
        ...
        let root = temp_root.join(format!(
            "devil_desktop_remote_workspace_gui_{}_{}_{}",
            std::process::id(),
            nanos,
            id
        ));
...
impl Drop for TempWorkspace {
    fn drop(&mut self) {
        ...
        if self.root.starts_with(&temp_root)
            && file_name.is_some_and(|name| name.starts_with("devil_desktop_remote_workspace_gui_"))
        {
...
fn remote_snapshot() -> devil_ui::ShellProjectionSnapshot {
    ...
    snapshot.remote_gui_projection = RemoteGuiProjection {
        ...
        session_rows: vec![
            RemoteWorkspaceSessionGuiRow {
                ...
                agent_version: "devil-remote-test/1".to_string(),
