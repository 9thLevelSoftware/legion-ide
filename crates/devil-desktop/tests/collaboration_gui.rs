...
impl TempWorkspace {
    fn new() -> Self {
        ...
        let root = temp_root.join(format!(
            "devil_desktop_collaboration_gui_{}_{}_{}",
            std::process::id(),
            nanos,
            id
        ));
...
impl Drop for TempWorkspace {
    fn drop(&mut self) {
        ...
        if self.root.starts_with(&temp_root)
            && file_name.is_some_and(|name| name.starts_with("devil_desktop_collaboration_gui_"))
        {
...
fn collaboration_snapshot() -> devil_ui::ShellProjectionSnapshot {
