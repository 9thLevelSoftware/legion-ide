...
use devil_protocol::{BufferId, TerminalPanelProjection, TerminalSessionId, TextCoordinate};
use devil_protocol::{
...
impl TempWorkspace {
    fn new() -> Self {
        let root = std::env::temp_dir().join(format!(
            "devil-desktop-language-terminal-workflow-{}-{}",
            std::process::id(),
...
fn desktop_phase4_dispatch_stays_projection_only() {
    ...
    for source in [bridge_source, view_source] {
        ...
        assert!(!source.contains("devil_terminal"));
