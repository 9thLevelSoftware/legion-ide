...
fn package_plan_points_at_expected_debug_executable() {
    ...
    assert_eq!(plan.cargo_args, ["build", "-p", "devil-desktop"]);
    assert_eq!(
...
fn package_manifest_is_metadata_only_and_redacts_source_payloads() {
    ...
    assert!(manifest.contains("package: devil-desktop"));
    assert!(manifest.contains("platform: windows"));
    assert!(manifest.contains("dry_run: true"));
    assert!(manifest.contains("cargo_command: cargo build -p devil-desktop"));
...
fn package_manifest_write_creates_only_manifest_metadata() {
    let root = workspace_root();
    let output =
        std::env::temp_dir().join(format!("devil_desktop_package_test_{}", std::process::id()));
    ...
    if output.starts_with(std::env::temp_dir())
        && output
            ...
            .is_some_and(|name| name.starts_with("devil_desktop_package_test_"))
    {
