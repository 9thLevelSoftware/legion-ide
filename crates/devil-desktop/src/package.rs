...
/// The Windows executable produced by the `devil-desktop` package.
pub const WINDOWS_EXECUTABLE_NAME: &str = "devil-desktop.exe";

/// The manifest file written into dry-run or package output directories.
pub const PACKAGE_MANIFEST_NAME: &str = "devil-desktop-package-manifest.txt";
...
impl PackageProfile {
    ...
    pub fn cargo_args(self) -> Vec<&'static str> {
        let mut args = vec!["build", "-p", "devil-desktop"];
