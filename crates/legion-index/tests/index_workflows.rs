...
fn metadata_record_for(document: &SourceDocument) -> legion_protocol::SemanticMetadataRecord {
    LexicalIndexer::new()
        .index_document(
            document,
...
) -> legion_protocol::LspWorkspaceTrustPosture {
    legion_protocol::LspWorkspaceTrustPosture {
        workspace_id: WorkspaceId(55),
...
fn lsp_request_correlation(identity: &SemanticFileFingerprintIdentity) -> LspRequestCorrelation {
    LspRequestCorrelation {
        request_id: legion_protocol::LspRequestId(Uuid::from_u128(4307)),
        server_id: LanguageServerId(7),
        workspace_id: identity.workspace_id,
...
fn repository_discovery_importer_invalidates_deleted_records_from_workspace_delta() {
    let delta = WorkspaceDiscoveryDelta {
        ...
        sequence: legion_protocol::EventSequence(9),
...
fn retrieval_chunk(
    ...
    file_id: FileId,
    label: &str,
) -> legion_index::RetrievalChunkRecord {
    index
        .retrieval_chunks()
        .iter()
