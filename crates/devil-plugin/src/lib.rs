...
impl PluginRuntimeHost {
    ...
    pub fn load_manifest(&mut self, manifest: PluginManifest) -> ProtocolResult<PluginId> {
        ...
        if !matches!(
            ...
            devil_protocol::PluginTrustDecision::Trusted
                | devil_protocol::PluginTrustDecision::ExplicitlyAllowed
        ) {
    ...
    pub fn dispatch_host_call(
        ...
        let decision = self.broker.decide_with_request_context(
            TrustState::Trusted,
            devil_protocol::PrincipalId(format!("plugin:{}", request.plugin_id.0)),
            request.declared_capability.clone(),
            None,
            devil_protocol::CapabilityRequestContext {
                plugin_namespace: Some(devil_protocol::CapabilityNamespace(format!(
                    "plugin.{}",
                    request.plugin_id.0
                ))),
        ...
        if let devil_security::SecurityDecision::Deny(message) = decision {
            return Ok(PluginHostCallResponse::Denied {
                reason: PluginDenialReason::MissingCapability,
                message,
            });
