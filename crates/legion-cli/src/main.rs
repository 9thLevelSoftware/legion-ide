...
use legion_protocol::{PRODUCT_NAME, PRODUCT_SHORT_NAME};
use legion_storage::FileBackedStorage;
...
fn run_doctor(workspace: PathBuf) -> Result<()> {
    ...
    // Plan Phase 0/4/8: phase status must match the accepted evidence ledger.
    ...
    if let Some(ledger) = ledger {
        ...
        require_text(
            &ledger,
            "Phase 8 — Hardening | **Accepted**",
            "Phase 8 accepted status",
            &mut issues,
        );
        require_text(
            &ledger,
            "Phase 8 acceptance: Accepted.",
            "Phase 8 accepted evidence marker",
            &mut issues,
        );
    }
    ...
    require_phase4_runtime_boundaries(&workspace, "crates/legion-agent", &mut issues);
    require_phase4_runtime_boundaries(&workspace, "crates/legion-tracker", &mut issues);
    require_phase4_runtime_boundaries(&workspace, "crates/legion-memory", &mut issues);

    if issues.is_empty() {
        println!("{PRODUCT_SHORT_NAME} CLI doctor: OK");
        println!("Workspace: {}", workspace.display());
        println!("Next setup command: cargo run -p legion-cli -- phase-gates");
        return Ok(());
    }
...
fn run_setup_status(workspace: PathBuf) -> Result<()> {
    ...
    println!("cargo run -p legion-cli -- doctor");
    println!("cargo run -p legion-cli -- evidence check --phase phase0");
    println!("cargo run -p legion-cli -- evidence check --phase phase3");
    println!("cargo run -p legion-cli -- evidence check --phase gui-phase6");
    println!("cargo run -p legion-cli -- evidence check --phase gui-phase7");
    println!("cargo run -p legion-cli -- evidence check --phase phase8");
...
fn validate_artifact_markers(
    ...
    for marker in markers {
        if !contains_current_or_historical_marker(&source, marker) {
            issues.push(format!(
...
fn require_command_marker(contents: &str, command: &str, label: &str, issues: &mut Vec<String>) {
    if !contains_current_or_historical_marker(contents, command) {
        issues.push(format!("missing marker for {label}: `{command}`"));
    }
}

fn contains_current_or_historical_marker(contents: &str, marker: &str) -> bool {
    ...
    historical != marker && contents.contains(&historical)
}
...
fn require_phase4_runtime_boundaries(
    ...
    if !source.contains("legion_protocol") {
        issues.push(format!(
...
"#
    ...
    fn gui_phase6_not_accepted_scaffold_passes_without_artifacts() {
        let workspace = TempWorkspace::new("legion_cli_gui_phase6_not_accepted");
        workspace.write(
...
"#,
    ...
    fn gui_phase6_acceptance_requires_artifacts_commands_and_checked_checklist() {
        let workspace = TempWorkspace::new("legion_cli_gui_phase6_incomplete");
        workspace.write(
            GUI_PHASE6_EVIDENCE_PATH,
    ...
    fn gui_phase6_acceptance_passes_with_artifacts_commands_and_checked_checklist() {
        let workspace = TempWorkspace::new("legion_cli_gui_phase6_complete");
        workspace.write(
            GUI_PHASE6_EVIDENCE_PATH,
    ...
    fn gui_phase7_not_accepted_scaffold_passes_without_artifacts() {
        let workspace = TempWorkspace::new("legion_cli_gui_phase7_not_accepted");
        workspace.write(
...
"#,
    ...
    fn gui_phase7_acceptance_requires_artifacts_commands_limits_and_checked_checklist() {
        let workspace = TempWorkspace::new("legion_cli_gui_phase7_incomplete");
        ...
        assert!(issues.iter().any(|issue| {
            issue.contains(
                "missing marker for GUI Phase 7 required command `cargo run -p legion-cli -- evidence check --phase gui-phase7` is listed",
            )
        }));
        assert!(issues.iter().any(|issue| {
            issue.contains(
    ...
    fn gui_phase7_acceptance_passes_with_artifacts_commands_limits_and_checked_checklist() {
        let workspace = TempWorkspace::new("legion_cli_gui_phase7_complete");
        workspace.write(
            GUI_PHASE7_EVIDENCE_PATH,
    ...
    fn gui_phase8_not_accepted_scaffold_passes_without_artifacts() {
        let workspace = TempWorkspace::new("legion_cli_gui_phase8_not_accepted");
        workspace.write(
...
"#,
    ...
    fn gui_phase8_acceptance_requires_artifacts_commands_markers_and_checked_checklist() {
        let workspace = TempWorkspace::new("legion_cli_gui_phase8_incomplete");
        ...
        assert!(issues.iter().any(|issue| {
            issue.contains(
                "missing marker for GUI Phase 8 required command `cargo run -p legion-cli -- evidence check --phase gui-phase8` is listed",
            )
        }));
        assert!(issues.iter().any(|issue| {
            issue.contains(
    ...
    fn gui_phase8_acceptance_rejects_phase7_unsupported_labels() {
        let workspace = TempWorkspace::new("legion_cli_gui_phase8_stale_label");
    ...
    fn gui_phase8_acceptance_passes_with_artifacts_commands_markers_and_checked_checklist() {
        let workspace = TempWorkspace::new("legion_cli_gui_phase8_complete");
        workspace.write(
            GUI_PHASE8_EVIDENCE_PATH,
