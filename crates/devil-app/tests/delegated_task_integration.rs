...
use devil_app::{AppComposition, AppDelegatedTaskExecutionOutcome, AppProductMode};
use devil_protocol::{
...
fn delegate_chat_projects_rag_citations_without_raw_source_payload() {
    ...
    assert!(outcome.projection.chat_messages.iter().any(|message| {
        message.role == devil_protocol::DelegatedTaskChatRole::Assistant
            && message
                .content_label
