...
fn open_rust_file(app: &mut AppComposition, root: &std::path::Path) -> devil_protocol::BufferId {
...
fn inline_projection(outcome: AppCommandOutcome) -> devil_ui::AssistInlinePredictionProjection {
    match outcome {
...
fn assist_inline_prediction_request_projects_and_accepts_bounded_ghost_text() {
    ...
    assert_eq!(
        active.status,
        devil_ui::AssistInlinePredictionStatusProjection::Ready
    );
    ...
    assert!(accepted.rows.iter().any(|row| {
        row.prediction_id == active.prediction_id
            && row.status == devil_ui::AssistInlinePredictionStatusProjection::Accepted
    }));
    assert!(
        app.editor()
            .text(buffer_id)
...
fn assist_inline_prediction_accept_marks_stale_without_mutating_after_buffer_change() {
    ...
    assert!(stale.rows.iter().any(|row| {
        row.prediction_id == prediction_id
            && row.status == devil_ui::AssistInlinePredictionStatusProjection::Stale
    }));
...
fn assist_inline_prediction_projection_hides_ghost_text_when_buffer_becomes_stale() {
    ...
    assert_eq!(
        stale_row.status,
        devil_ui::AssistInlinePredictionStatusProjection::Stale
    );
