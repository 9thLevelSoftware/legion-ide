...
use legion_protocol::{BufferId, FileId, ProtocolTextRange, TextCoordinate, ViewportScroll};
use legion_ui::ExplorerNodeProjection;
...
impl TempWorkspace {
    fn new() -> Self {
        ...
        let root = temp_root.join(format!(
            "legion_desktop_daily_editing_{}_{}_{}",
            std::process::id(),
            nanos,
            id
        ));
...
impl Drop for TempWorkspace {
    fn drop(&mut self) {
        ...
        if self.root.starts_with(&temp_root)
            && file_name.is_some_and(|name| name.starts_with("legion_desktop_daily_editing_"))
        {
