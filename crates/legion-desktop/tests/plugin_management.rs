...
impl TempWorkspace {
    fn new() -> Self {
        ...
        let root = temp_root.join(format!(
            "legion_desktop_plugin_management_{}_{}_{}",
            std::process::id(),
            nanos,
            id
        ));
...
impl Drop for TempWorkspace {
    fn drop(&mut self) {
        ...
        if self.root.starts_with(&temp_root)
            && file_name.is_some_and(|name| name.starts_with("legion_desktop_plugin_management_"))
        {
...
fn plugin_snapshot() -> legion_ui::ShellProjectionSnapshot {
