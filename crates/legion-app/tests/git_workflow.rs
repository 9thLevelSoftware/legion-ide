...
impl Drop for TempGitRepo {
    fn drop(&mut self) {
        ...
        if self.root.starts_with(&temp_root)
            && file_name.is_some_and(|name| name.starts_with("legion_app_git_"))
        {
...
fn git_workflow_refreshes_projection_and_stages_hunks_through_app_authority() {
    ...
    app.open_workspace(
        ...
        legion_protocol::PrincipalId("git-test".to_string()),
    )
...
#[test]
fn git_workflow_commits_staged_changes_through_app_authority() {
    let repo = TempGitRepo::new();
    ...
    assert_eq!(
        run_git(repo.path(), ["log", "-1", "--pretty=%s"]).trim(),
        "feat: update alpha"
    );
    assert!(
        run_git(repo.path(), ["diff", "--cached", "--", "src/lib.rs"])
...
fn git_workflow_resolves_conflicts_through_app_authority() {
    ...
    app.open_workspace(
        repo.path(),
        legion_protocol::WorkspaceTrustState::Trusted,
        legion_protocol::PrincipalId("git-conflict-test".to_string()),
    )
    .expect("workspace should open");
    app.open_file(source.to_string_lossy())
        .expect("source should open");
...
fn git_workflow_resolves_conflict_and_syncs_open_buffer() {
    ...
    assert!(
        matches!(save_outcome, legion_app::AppSaveOutcome::Saved(_)),
...
fn git_workflow_rejects_conflict_resolution_from_subdirectory_when_buffer_dirty() {
    ...
    app.open_workspace(
        &subdir,
        legion_protocol::WorkspaceTrustState::Trusted,
        legion_protocol::PrincipalId("git-conflict-subdir-dirty-test".to_string()),
    )
...
fn git_workflow_resolves_conflict_from_subdirectory_and_syncs_open_buffer() {
    ...
    app.open_workspace(
        &subdir,
        legion_protocol::WorkspaceTrustState::Trusted,
        legion_protocol::PrincipalId("git-conflict-subdir-sync-test".to_string()),
    )
    .expect("workspace should open from subdirectory");
    // Open the file using the absolute repo path.
    app.open_file(source.to_string_lossy())
