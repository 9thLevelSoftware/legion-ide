...
use legion_protocol::{PrincipalId, ProposalResponse, WorkspaceTrustState};
...
impl TempWorkspace {
    fn new() -> Self {
        ...
        let root = std::env::temp_dir().join(format!(
            "legion_app_structural_search_{}_{}_{}",
            std::process::id(),
            nanos,
            id
        ));
...
impl Drop for TempWorkspace {
    fn drop(&mut self) {
        ...
        if self.root.starts_with(&temp_root)
            && file_name.is_some_and(|name| name.starts_with("legion_app_structural_search_"))
        {
...
fn run_structural_search(
    ...
    pattern: &str,
    rewrite: Option<&str>,
) -> legion_ui::StructuralSearchProjection {
    match app
