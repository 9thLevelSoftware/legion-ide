...
impl AppProductMode {
    fn allows_assist(self) -> bool {
        self.to_product_mode()
            .allows_runtime_surface(legion_protocol::ProductRuntimeSurface::AssistedAi)
    }

    fn allows_delegate(self) -> bool {
        self.to_product_mode()
            .allows_runtime_surface(legion_protocol::ProductRuntimeSurface::DelegatedTask)
    }

    fn allows_automate(self) -> bool {
        self.to_product_mode()
            .allows_runtime_surface(legion_protocol::ProductRuntimeSurface::Automation)
    }

    /// Convert the app authority enum into the shared protocol product mode.
    pub fn to_product_mode(self) -> legion_protocol::ProductMode {
        match self {
...
pub struct BatchRollbackStepContract {
    ...
    pub action: legion_protocol::ProposalRollbackAction,
...
impl AppProposalCoordinator {
    ...
    fn next_id(&self) -> legion_protocol::ProposalId {
    ...
    fn proposal_ledger_projection(
        &self,
        generated_at: TimestampMillis,
    ) -> legion_protocol::ProposalLedgerProjection {
        let proposals = self.proposals.borrow();
        let states = self.proposal_states.borrow();
    ...
    fn proposal_ledger_row(
        ...
        state: ProposalLifecycleState,
        generated_at: TimestampMillis,
    ) -> legion_protocol::ProposalLedgerRow {
        let target_coverage = legion_protocol::proposal_metadata_target_coverage(proposal);
        let payload_kind = legion_protocol::proposal_payload_kind(&proposal.payload);
        ...
        {
            legion_protocol::ProposalPrivacyLabel::RedactedSensitive
        } else {
    ...
    fn lifecycle_state_display(
        state: ProposalLifecycleState,
    ) -> legion_protocol::ProposalLifecycleStateDisplay {
        legion_protocol::ProposalLifecycleStateDisplay {
            state,
            label: format!("{state:?}"),
    ...
    fn bounded_proposal_title(
        proposal: &WorkspaceProposal,
        payload_kind: legion_protocol::ProposalPayloadKind,
    ) -> String {
    ...
    fn rollback_availability(
        payload: &ProposalPayload,
    ) -> legion_protocol::ProposalRollbackAvailability {
        match payload {
                ...
                ProposalBatchRollbackPolicy::Required => {
                    if batch.rollback_steps.is_empty()
                        || batch.rollback_steps.iter().any(|step| {
                            step.action == legion_protocol::ProposalRollbackAction::Unsupported
                        })
                    {
                        legion_protocol::ProposalRollbackAvailability::Unavailable
                    } else {
                        legion_protocol::ProposalRollbackAvailability::Available
                    }
                }
                ProposalBatchRollbackPolicy::BestEffort => {
                    legion_protocol::ProposalRollbackAvailability::BestEffort
                }
                ProposalBatchRollbackPolicy::NotSupported => {
                    legion_protocol::ProposalRollbackAvailability::Unavailable
                }
                ProposalBatchRollbackPolicy::NotRequired => {
                    legion_protocol::ProposalRollbackAvailability::NotRequired
                }
            },
            ProposalPayload::WorkspaceEdit(_) => {
                legion_protocol::ProposalRollbackAvailability::BestEffort
            ...
            | ProposalPayload::CodeAction(_) => {
                legion_protocol::ProposalRollbackAvailability::Unknown
            }
        }
    }
    ...
    fn context_manifest_summary(
        ...
        target_coverage: &ProposalTargetCoverage,
        privacy_label: legion_protocol::ProposalPrivacyLabel,
    ) -> legion_protocol::ProposalContextManifestSummary {
        let item_count = target_coverage.targets.len() as u32;
        let categories = if item_count == 0 && target_coverage.omitted_target_count == 0 {
            Vec::new()
        } else {
            vec![legion_protocol::ProposalContextManifestEntrySummary {
    ...
    fn diff_summary(
        payload: &ProposalPayload,
        target_coverage: &ProposalTargetCoverage,
    ) -> legion_protocol::ProposalDiffSummary {
        let (kind, hunk_count) = match payload {
            ProposalPayload::TextEdit(payload) => (
                legion_protocol::ProposalDiffSummaryKind::Text,
                payload.edits.edits.len() as u32,
            ),
            ProposalPayload::CreateFile(_)
            | ProposalPayload::DeleteFile(_)
            | ProposalPayload::RenameFile(_) => {
                (legion_protocol::ProposalDiffSummaryKind::FileOperation, 1)
            }
            ProposalPayload::WorkspaceEdit(payload) => (
                legion_protocol::ProposalDiffSummaryKind::WorkspaceEdit,
                payload
                    .file_edits
                    .iter()
            ...
            ProposalPayload::SaveFile(_)
            | ProposalPayload::FormatFile(_)
            | ProposalPayload::CodeAction(_) => (legion_protocol::ProposalDiffSummaryKind::Text, 0),
            ProposalPayload::TerminalCommand(_) => (
        ...
        legion_protocol::ProposalDiffSummary {
            ...
            chunks: target_coverage
                ...
                .enumerate()
                .map(
                    |(index, target)| legion_protocol::ProposalDiffChunkDescriptor {
        ...
        match payload {
            ...
            ProposalPayload::WorkspaceEdit(payload) => {
                ...
                for operation in &payload.file_operations {
                    match operation {
                        legion_protocol::WorkspaceFileOperation::Create { path, .. } => {
                            visitor(Self::path_target(
                        ...
                        legion_protocol::WorkspaceFileOperation::Delete { file } => {
                            visitor(Self::file_identity_target(
                        ...
                        legion_protocol::WorkspaceFileOperation::Rename { file, destination } => {
                            visitor(Self::file_identity_target(
    ...
    fn file_identity_target(
        ...
        byte_ranges: Vec<legion_protocol::ByteRange>,
    ...
    fn push_single_target_validation_diagnostics(
        ...
        if target.terminal_session_id == Some(legion_protocol::TerminalSessionId(0)) {
            diagnostics.push(Self::diagnostic(
    ...
    fn push_payload_validation_diagnostics(
        ...
        match &proposal.payload {
            ...
            ProposalPayload::WorkspaceEdit(payload) => {
                ...
                for operation in &payload.file_operations {
                    match operation {
                        ...
                        legion_protocol::WorkspaceFileOperation::Delete { .. }
                        | legion_protocol::WorkspaceFileOperation::Rename { .. } => {
                            Self::push_file_precondition_diagnostics(
                                &proposal.preconditions,
                                diagnostics,
                ...
                if payload.source == legion_protocol::WorkspaceEditSourceKind::Plugin {
                    diagnostics.push(Self::diagnostic(
            ...
            ProposalPayload::Batch(payload) => {
                ...
                if payload.rollback_policy == legion_protocol::ProposalBatchRollbackPolicy::Required
                ...
                if payload.rollback_policy
                    == legion_protocol::ProposalBatchRollbackPolicy::NotSupported
                    && payload.atomicity
                        != legion_protocol::ProposalBatchAtomicity::OrderedNonAtomic
                {
                    diagnostics.push(Self::diagnostic(
                ...
                for item in &payload.items {
                    ...
                    if payload.rollback_policy
                        == legion_protocol::ProposalBatchRollbackPolicy::Required
    ...
    fn command_action_matches_request(command: &ProposalLifecycleCommand, action: &str) -> bool {
        matches!(
            ...
            (legion_protocol::ProposalLifecycleAction::Approve, "approve")
                | (legion_protocol::ProposalLifecycleAction::Reject, "reject")
                | (legion_protocol::ProposalLifecycleAction::Cancel, "cancel")
                | (
                    legion_protocol::ProposalLifecycleAction::Rollback,
                    "rollback"
                )
        )
    }
...
impl TerminalWorkflow {
    ...
    fn record_audit(
        ...
        self.last_audit = Some(legion_protocol::TerminalAuditRecord {
            session_id,
            state,
            event_sequence,
            correlation_id: event_context.correlation_id,
...
impl ProjectionBuilder {
    fn active_buffer_projection(
        ...
        let request = legion_protocol::EditorViewportRequest {
            buffer_id,
    ...
    fn failed_response_for_editor_error(
        ...
        let proposal = WorkspaceProposal {
            ...
            payload: ProposalPayload::SaveFile(SaveFileProposal {
                ...
                buffer_version: legion_protocol::BufferVersion(0),
...
fn empty_context_manifest_projection() -> legion_protocol::ContextManifestProjection {
    legion_protocol::ContextManifestProjection {
        manifest: legion_protocol::ContextManifestRecord {
            manifest_id: "manifest:empty".to_string(),
            workspace_id: None,
            proposal_id: None,
            purpose: legion_protocol::ContextManifestPurpose::TrustReview,
            workspace_trust_state: None,
            privacy_label: legion_protocol::ProposalPrivacyLabel::PublicMetadata,
            risk_label: legion_protocol::ProposalRiskLabel::Informational,
            egress: legion_protocol::ContextManifestEgressStatus::LocalOnly,
            items: Vec::new(),
            permissions: Vec::new(),
            omitted_item_count: 0,
...
fn empty_privacy_inspector_projection() -> legion_protocol::PrivacyInspectorProjection {
    legion_protocol::PrivacyInspectorProjection {
...
fn empty_permission_budget_projection() -> legion_protocol::PermissionBudgetProjection {
...
fn empty_approval_checklist_projection() -> legion_protocol::ProposalApprovalChecklistProjection {
    legion_protocol::ProposalApprovalChecklistProjection {
...
fn empty_checkpoint_rollback_projection() -> legion_protocol::CheckpointRollbackProjection {
    legion_protocol::CheckpointRollbackProjection {
        ...
        checkpoint: legion_protocol::ProposalCheckpointProjection {
            ...
            expected_preconditions:
                legion_protocol::ContextManifestPreconditionSummary::from_preconditions(
...
fn empty_assisted_ai_projection() -> legion_protocol::AssistedAiProjection {
    legion_protocol::AssistedAiProjection {
...
struct SelectedProposalTrustProjections {
    ...
    approval_checklist_projection: legion_protocol::ProposalApprovalChecklistProjection,
    checkpoint_rollback_projection: legion_protocol::CheckpointRollbackProjection,
}
...
struct AppCommandDescriptorInput<'a> {
    ...
    risk_label: legion_protocol::CommandRiskLabel,
    required_permission: Option<CapabilityId>,
    target: Option<String>,
}

fn command_descriptor(input: AppCommandDescriptorInput<'_>) -> legion_protocol::CommandDescriptor {
    legion_protocol::CommandDescriptor {
...
fn assisted_ai_request_contract_from_metadata(
    request_id: String,
    provider_capability: &legion_protocol::AssistedAiProviderCapability,
    ...
    generated_at: TimestampMillis,
) -> legion_protocol::AssistedAiRequestContract {
    legion_protocol::AssistedAiRequestContract {
        request_id,
        ...
        permission_budget_evaluations: permission_budget_projection
            ...
            .iter()
            .map(|evaluation| {
                legion_protocol::AssistedAiPermissionBudgetEvaluationReference::from_evaluation(
                    evaluation,
        ...
        checkpoint_rollback: checkpoint_rollback_projection.map(|projection| {
            trust_reference(
                ...
                legion_protocol::AssistedAiTrustProjectionKind::CheckpointRollback,
            )
        }),
        correlation_id: event_context.correlation_id,
        causality_id: event_context.causality_id,
...
pub enum AppCommandOutcome {
    ...
    AiRunCancelled(legion_protocol::AgentRunId),
    /// Phase 4 AI run replay metadata was loaded.
    AiRunReplayed(Box<legion_protocol::AgentReplayManifest>),
...
pub struct AppAiRunOutcome {
    ...
    pub run_id: legion_protocol::AgentRunId,
    ...
    pub route_response: legion_protocol::AssistedAiProviderRouteResponse,
    /// Context manifest projection used before provider invocation.
    pub context_manifest_projection: legion_protocol::ContextManifestProjection,
    /// Privacy inspector projection derived from the manifest.
    pub privacy_inspector_projection: legion_protocol::PrivacyInspectorProjection,
    /// Permission budget projection evaluated before provider invocation.
    pub permission_budget_projection: legion_protocol::PermissionBudgetProjection,
    /// Visible refusal metadata for denied or unavailable routes.
    pub refusal: Option<legion_protocol::AssistedAiRefusalMetadata>,
    /// Replay manifest persisted for the run.
    pub replay_manifest: legion_protocol::AgentReplayManifest,
}
...
pub struct AppAiInspectionSnapshot {
    ...
    pub assisted_ai_projection: legion_protocol::AssistedAiProjection,
}

#[derive(Debug, Clone, Default)]
struct Phase4ProjectionState {
    context_manifest_projection: Option<legion_protocol::ContextManifestProjection>,
    privacy_inspector_projection: Option<legion_protocol::PrivacyInspectorProjection>,
    permission_budget_projection: Option<legion_protocol::PermissionBudgetProjection>,
...
struct SharedProposalGate {
    ...
    applied_operation_ids: Vec<legion_protocol::CollaborationOperationId>,
    stale: bool,
}
...
struct LegionCloudLaneComposition {
    ...
    network_target: Option<legion_protocol::NetworkTarget>,
    security_broker: DenyByDefaultBroker,
...
fn parse_cloud_lane_endpoint(
    endpoint: &str,
) -> Result<legion_protocol::NetworkTarget, AppCompositionError> {
...
fn app_cloud_contract_error(
    error: legion_protocol::AssistedAiContractError,
) -> AppCompositionError {
    AppCompositionError::Remote(app_cloud_contract_reason(&error))
}

fn app_cloud_contract_reason(error: &legion_protocol::AssistedAiContractError) -> String {
    match error {
        legion_protocol::AssistedAiContractError::NonMetadataOnlyAuditRecord { field, reason }
...
impl AppComposition {
    ...
    pub fn invoke_plugin_command(
        ...
        let envelope = plugin_event_envelope(
            legion_protocol::EventId(uuid::Uuid::now_v7()),
            plugin_id,
    ...
    pub fn receive_collaboration_transport_envelope(
        ...
        if outcome.acknowledgement.status
            != legion_protocol::CollaborationAcknowledgementStatus::Accepted
        {
            return Ok(None);
        }
        ...
        let record = self
            ...
            .apply_protocol_edits(EditorApplyTransactionRequest {
                ...
                edits: legion_protocol::EditBatch {
                    edits: vec![legion_protocol::TextEdit { range, replacement }],
                },
    ...
    fn collaboration_audit_record(
        ...
        operation_id: Option<legion_protocol::CollaborationOperationId>,
        proposal_id: Option<ProposalId>,
    ...
    pub fn connect_remote_workspace_session(
        ...
        let spec = self.remote_connection_spec(
            ...
            "legion-remote-ssh-agent/1".to_string(),
        )?;
    ...
    pub fn connect_devcontainer_workspace_session_from_json(
        ...
        let spec = self.remote_connection_spec(
            session_id,
            authority_label.into(),
            "legion-remote-devcontainer-agent/1".to_string(),
        )?;
        let plan =
    ...
    fn remote_connection_spec(
        ...
        let context = self.active_documents.require_workspace_context()?;
        let authority_id = legion_protocol::RemoteAuthorityId(session_id.0.saturating_add(100));
        ...
        Ok(RemoteConnectionSpec {
            session_id,
            authority_id,
            agent_id: legion_protocol::RemoteAgentId(session_id.0.saturating_add(200)),
            workspace_id: context.workspace_id,
            principal_id: context.principal,
            authority_label,
    ...
    pub fn execute_delegated_task(
        ...
        let run_id = legion_protocol::AgentRunId(format!("run-{}", plan_id.0));
    ...
    pub fn seed_legion_workflow_sessions(
        ...
        for session in sessions {
            if session.session_id.0.trim().is_empty()
                ...
                || legion_protocol::validate_legion_workflow_session(&session).is_err()
            {
                continue;
            }
    ...
    pub fn legion_workflow_projection(
        ...
        let mut projection = legion_protocol::legion_workflow_projection_from_sessions(
        ...
        self.legion_workflow_tracker_ledger
            .append(LegionWorkflowTrackerRecord {
                ...
                risk_labels: session
                    ...
                    .collect(),
                privacy_labels: vec![legion_protocol::PrivacyClassification::Metadata],
    ...
    fn delegated_task_projection(
        ...
        let mut projection = legion_protocol::delegated_task_projection_from_plan_contracts(
        ...
        legion_protocol::CommandRegistryProjection {
            ...
            commands: vec![
                ...
                command_descriptor(AppCommandDescriptorInput {
                    ...
                    shortcut: None,
                    risk_label: legion_protocol::CommandRiskLabel::Privileged,
    ...
    fn artifact_ledger_projection(
        ...
        delegated_task_projection: &DelegatedTaskProjection,
        generated_at: TimestampMillis,
    ) -> legion_protocol::ArtifactLedgerProjection {
        let mut rows = delegated_task_projection
            .plan_rows
            .iter()
            .map(|plan| legion_protocol::ArtifactLedgerRow {
        ...
        rows.extend(proposal_ledger_projection.rows.iter().map(|proposal| {
            legion_protocol::ArtifactLedgerRow {
                artifact_id: format!("artifact:approval:{}", proposal.proposal_id.0),
                kind: legion_protocol::ArtifactKind::Approval,
        ...
        let rows = delegated_task_projection
            ...
            .map(|plan| legion_protocol::VerificationRunRow {
                ...
                state: if plan.readiness
                    == legion_protocol::DelegatedTaskPlanReadinessStatus::Refused
                    || plan.readiness == legion_protocol::DelegatedTaskPlanReadinessStatus::Blocked
                {
                    legion_protocol::VerificationRunState::Blocked
                } else {
                    legion_protocol::VerificationRunState::Planned
                },
    ...
    fn system_graph_projection(
        ...
        delegated_task_projection: &DelegatedTaskProjection,
        generated_at: TimestampMillis,
    ) -> legion_protocol::SystemGraphProjection {
        let mut nodes = vec![legion_protocol::SystemGraphNode {
        ...
        if self.active_documents.active_file_id.is_some() {
            nodes.push(legion_protocol::SystemGraphNode {
        ...
        let edges = nodes
            ...
            .map(|node| legion_protocol::SystemGraphEdge {
    ...
    ) -> Result<legion_protocol::RemoteFilesystemSnapshot, AppCompositionError> {
        self.remote
            .runtime
    ...
    pub fn cancel_ai_run(
        ...
        let record = legion_protocol::Phase4RuntimeAuditRecord {
            ...
            invocation_state: legion_protocol::AssistedAiProviderInvocationState::Cancelled,
    ...
    pub fn replay_ai_run(
        &self,
        run_id: legion_protocol::AgentRunId,
    ) -> Result<legion_protocol::AgentReplayManifest, AppCompositionError> {
        match self
            .storage
    ...
    fn persist_phase4_runtime_records(
        ...
        run_id: &legion_protocol::AgentRunId,
        route_id: &str,
        invocation_state: legion_protocol::AssistedAiProviderInvocationState,
        outcome_label: &str,
        event_context: EventContext,
    ...
    fn selected_proposal_trust_projections(
        &self,
        proposal_ledger_projection: &legion_protocol::ProposalLedgerProjection,
        generated_at: TimestampMillis,
        ...
        let causality_id = self
            ...
            .map(|context| context.causality_id);
        let audit_status =
            if row.rollback == legion_protocol::ProposalRollbackAvailability::NotRequired {
                legion_protocol::CheckpointRollbackAuditStatus::NotRequired
            } else {
                legion_protocol::CheckpointRollbackAuditStatus::Available
            };
        let checkpoint_rollback_projection =
    ...
    fn selected_proposal_permission_budget_projection(
        proposal: &WorkspaceProposal,
        context_manifest_projection: &legion_protocol::ContextManifestProjection,
        risk_label: legion_protocol::ProposalRiskLabel,
        generated_at: TimestampMillis,
    ) -> legion_protocol::PermissionBudgetProjection {
        let action_class = match &proposal.payload {
            ProposalPayload::TerminalCommand(_) => {
                legion_protocol::PermissionBudgetActionClass::AccessTerminal
            }
            _ => legion_protocol::PermissionBudgetActionClass::ApplyApprovedProposal,
        };
        let budget = legion_protocol::PermissionBudgetContract {
        ...
        let action = context_manifest_projection
            ...
            .first()
            .map(|permission| {
                legion_protocol::permission_budget_action_from_permission_summary(
                    permission,
            ...
            .unwrap_or_else(|| legion_protocol::PermissionBudgetActionSummary {
    ...
    fn rollback_snapshot_for_workspace_edit(
        &self,
        proposal: &WorkspaceProposal,
        payload: &legion_protocol::WorkspaceEditProposalPayload,
    ) -> Result<ProposalMutationRollback, ProposalResponse> {
    ...
    fn rollback_snapshot_for_workspace_file_operation(
        &self,
        proposal: &WorkspaceProposal,
        operation: &legion_protocol::WorkspaceFileOperation,
    ) -> Result<ProposalMutationRollback, ProposalResponse> {
        match operation {
            legion_protocol::WorkspaceFileOperation::Create { path, .. } => self
                .workspace_rollback_checkpoint(
                    proposal,
                    WorkspaceMutationRollbackTarget::CreatedFile { path: path.clone() },
                ),
            legion_protocol::WorkspaceFileOperation::Delete { file } => self
                .workspace_rollback_checkpoint(
                    proposal,
                    WorkspaceMutationRollbackTarget::DeletedFile { file: file.clone() },
                ),
            legion_protocol::WorkspaceFileOperation::Rename { file, destination } => self
                .workspace_rollback_checkpoint(
                    proposal,
                    WorkspaceMutationRollbackTarget::RenamedFile {
                        file: file.clone(),
    ...
    fn stale_apply_response(
        ...
        match self
            ...
            Ok(transition) => ProposalResponse::Stale {
                transition,
                stale: legion_protocol::ProposalStaleContext {
                    reason,
    ...
    fn apply_text_edit_proposal(
        &mut self,
        proposal: &WorkspaceProposal,
        payload: &legion_protocol::TextEditProposal,
    ) -> ProposalResponse {
    ...
    fn apply_create_file_proposal(
        &mut self,
        proposal: &WorkspaceProposal,
        payload: &legion_protocol::CreateFileProposal,
    ) -> ProposalResponse {
        let workspace_id = match self.active_documents.require_workspace_id() {
            Ok(workspace_id) => workspace_id,
            Err(err) => {
    ...
    fn apply_delete_file_proposal(
        &mut self,
        proposal: &WorkspaceProposal,
        payload: &legion_protocol::DeleteFileProposal,
    ) -> ProposalResponse {
    ...
    fn apply_rename_file_proposal(
        &mut self,
        proposal: &WorkspaceProposal,
        payload: &legion_protocol::RenameFileProposal,
    ) -> ProposalResponse {
        if let Some(response) = self.reject_open_file_mutation(proposal, &payload.file) {
            return response;
        }
    ...
    fn preflight_workspace_text_edit(
        &self,
        proposal: &WorkspaceProposal,
        edit: &legion_protocol::WorkspaceTextEdit,
    ) -> Result<(), ProposalResponse> {
    ...
    fn preflight_workspace_file_operation(
        ...
        match operation {
            ...
            legion_protocol::WorkspaceFileOperation::Delete { file }
            | legion_protocol::WorkspaceFileOperation::Rename { file, .. } => {
    ...
    fn apply_workspace_file_operation(
        ...
        match operation {
            legion_protocol::WorkspaceFileOperation::Create { path, .. } => {
                let create = legion_protocol::CreateFileProposal {
                    path: path.clone(),
            ...
            legion_protocol::WorkspaceFileOperation::Delete { file } => {
                let delete = legion_protocol::DeleteFileProposal { file: file.clone() };
                self.apply_delete_file_proposal(proposal, &delete)
            }
            legion_protocol::WorkspaceFileOperation::Rename { file, destination } => {
                let rename = legion_protocol::RenameFileProposal {
                    file: file.clone(),
                    destination: destination.clone(),
                };
    ...
    fn apply_code_action_proposal(
        &mut self,
        proposal: &WorkspaceProposal,
        payload: &legion_protocol::CodeActionProposal,
    ) -> ProposalResponse {
        ...
        let edits = legion_protocol::EditBatch {
    ...
    fn append_dependency_blocked_failures(
        ...
        loop {
            ...
            for edge in &batch.dependency_edges {
                if !matches!(
                    edge.kind,
                    legion_protocol::ProposalBatchDependencyKind::RequiresValidation
                        | legion_protocol::ProposalBatchDependencyKind::RequiresApply
    ...
    fn rollback_action_matches_route(
        ...
        action: legion_protocol::ProposalRollbackAction,
    ) -> bool {
        matches!(
            (route, action),
            (
                BatchPreflightRoute::TextEdit,
                legion_protocol::ProposalRollbackAction::EditorUndoGroup
            ) | (
                BatchPreflightRoute::CreateFile,
                legion_protocol::ProposalRollbackAction::DeleteCreatedFile
            ) | (
                BatchPreflightRoute::DeleteFile,
                legion_protocol::ProposalRollbackAction::RecreateDeletedFile
            ) | (
                BatchPreflightRoute::RenameFile,
                legion_protocol::ProposalRollbackAction::RenamePathBack
            )
        )
    }
