...
impl LegionWorkflowOutcomeCandidate {
    ...
    pub fn from_session_metadata(
        ...
        let signed_off_count = session
            ...
            .iter()
            .filter(|signoff| {
                signoff.state == legion_protocol::LegionWorkflowSignOffState::SignedOff
            })
            .count();
        let passed_verification_count = session
            ...
            .iter()
            .filter(|gate| {
                gate.state == legion_protocol::LegionWorkflowVerificationGateState::Passed
            })
            .count();
        let candidate = Self {
...
mod tests {
    ...
    fn legion_workflow_session() -> LegionWorkflowSession {
        LegionWorkflowSession {
            ...
            generated_at: legion_protocol::TimestampMillis(1304),
            redaction_hints: vec![RedactionHint::MetadataOnly],
            schema_version: 1,
            correlation_id: CorrelationId(1),
    ...
    fn trace_record(consent: LegionTraceConsentState) -> LegionTraceRecord {
        LegionTraceRecord {
            ...
            generated_at: legion_protocol::TimestampMillis(8101),
    ...
    fn trace_export_manifest_requires_consent_and_hashes_only() {
        ...
        let manifest = service
            .trace_export_manifest(
                ...
                false,
                false,
                legion_protocol::TimestampMillis(8102),
            )
    ...
    fn trace_training_export_requires_hosted_training_consent() {
        ...
        let manifest = service
            .trace_export_manifest(
                ...
                true,
                true,
                legion_protocol::TimestampMillis(8103),
            )
