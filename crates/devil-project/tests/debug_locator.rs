...
use devil_project::{CargoDebugLocatorOptions, discover_cargo_debug_configurations};
...
impl TempCargoProject {
    fn new() -> Self {
        ...
        let root =
            std::env::temp_dir().join(format!("devil_debug_locator_{}_{}", std::process::id(), id));
...
impl Drop for TempCargoProject {
    fn drop(&mut self) {
        ...
        if self.root.starts_with(&temp_root)
            && file_name.is_some_and(|name| name.starts_with("devil_debug_locator_"))
        {
