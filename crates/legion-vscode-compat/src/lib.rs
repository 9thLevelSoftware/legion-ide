...
//! normalizes package metadata into `legion-protocol` DTOs so later product
...
mod tests {
    use super::*;
    use legion_protocol::RedactionHint;
    use serde_json::json;
    use uuid::Uuid;
    ...
    fn command_and_debug_manifest_is_policy_gated() {
        let manifest = manifest_from_package_json(
            ...
            json!({
                ...
                "version": "1.0.0",
                "activationEvents": ["onCommand:legion.run", "onDebug"],
                "contributes": {
    ...
    fn webview_manifest_is_deferred_to_tier3() {
        let manifest = manifest_from_package_json(
            ...
            json!({
                ...
                "version": "1.0.0",
                "contributes": {
                    "webviews": [{ "viewType": "legion.preview" }]
                }
            }),
            CorrelationId(3),
            causality_id(),
    ...
    fn invalid_manifest_and_control_ids_reject() {
        let missing = manifest_from_package_json(
            PluginId(10),
            json!({ "publisher": "legion", "version": "1.0.0" }),
            CorrelationId(4),
            causality_id(),
            EventSequence(4),
        );
        ...
        let bad_controls = manifest_from_package_json(
            PluginId(10),
            json!({ "publisher": "legion", "name": "bad", "version": "1.0.0" }),
            CorrelationId(0),
            causality_id(),
            EventSequence(4),
        );
    ...
    fn open_vsx_extension_metadata_resolves_and_loads_package_manifest() {
        ...
        assert_eq!(loaded.resolved.namespace, "legion");
        assert_eq!(loaded.manifest.extension_id.0, "legion.theme-fixture");
        assert_eq!(
    ...
    fn open_vsx_load_rejects_metadata_package_identity_mismatch() {
        let resolved = resolve_open_vsx_extension_metadata(
            ...
            &json!({
                ...
                "name": "theme-fixture",
                "version": "1.0.0",
                "downloadUrl": "https://open-vsx.org/api/legion/theme-fixture/1.0.0/file/legion.theme-fixture-1.0.0.vsix"
            }),
        )
        .expect("Open VSX metadata resolves");
